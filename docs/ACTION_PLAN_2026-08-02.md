...
`ac96d47` is complete and accepting**, satisfying the gate Codex set. Two
residues before launch: a lightweight Codex acknowledgement of FCR-001/002
(or an owner waiver — the product delta is one dead-state ingestion guard in
Stage 1 plus two tests), and the owner's stage-order choice — Stage 0
battery completion or Stage 1. None of this is deployed: the
