...
`afa47d9` after PR #226 merged Claude's REBAL-1 Stage 1 implementation.
Codex independently reviewed that exact merge on
`codex/review-rebal1-stage1-20260815`; product/test correction `5519a69`
closes five P2 and three P3 findings. Stage 1 is accepted after correction;
Stages 2 and 3 are not started. None of this is deployed: the operational
runtime remains separately frozen at `752d3b7` in active `paper-epoch-005`.
Any deployment of the changed code would close that epoch by changing its
`code_commit`, even though REBAL-1 Stage 1 does not change the mandate or
policy fingerprint.
...
| REBAL-1 | feature | **Owner request 2026-08-15: wide-band portfolio rebalancing.** The app had no portfolio-level target or drift surface; its only rebalancing was the leveraged-pair strategy. Codex authored the milestone plan and staged it; Claude implemented Stage 1. | **STAGE 1 COMPLETE AFTER INDEPENDENT CORRECTION 2026-08-15; merged implementation `afa47d9`, correction `5519a69`; not deployed.** Plan in `docs/REBAL1_MILESTONE_PLAN.md`; disposition and ledger in `docs/REVIEW_2026-08-15_REBAL1_STAGE1.md`. Stage 0 adds a versioned, fingerprinted SLEEVE allocation profile (`assistant/rebalance_profile.py`) whose targets the owner approved on 2026-08-15 (cash 10 / dividend 15 / growth 40 / leveraged 15 / hedge 10 / other 10, band +/-25% relative). Stage 1 adds a READ-ONLY Portfolio Rebalancing page with current and projected sleeve weights, target-relative bands, projected dollar gaps, residual exposure, and policy-feasibility disclosures. It emits no shares, sides, proposals, or approvals. The confirmed SOXX/SOXL wide-band result does not transfer to the general portfolio. Independent review closed five P2 and three P3 findings: pending orders now conserve cash and drive projected status/gaps; total-exposure and position-cap conflicts are named; malformed/unidentifiable orders fail closed; pending-only residual exposure stays visible; invalid profiles return unusable reports; nested profile state and config membership are hardened; and exact money remains Decimal in presentation. Corrected validation: 75 focused and 3,933 full-suite tests. Stages 2 (buy-only cash steering) and 3 (tax-aware trims, separate authorization) remain unstarted and unauthorized. |
| REBAL-2 | feature | **Stage 2 of the REBAL-1 plan: buy-only cash steering.** The owner supplies a new-money budget and picks one ticker per under-band sleeve; the app sizes the split and prepares APPROVE-gated buys. Also carries the owner's 2026-08-15 decision on the Stage 1 profile/policy conflict. | **IMPLEMENTED 2026-08-15; PENDING INDEPENDENT REVIEW; not merged, not deployed.** Branch `user/claude/rebal1-stage2-buy-steering-20260815`; disposition in `docs/REVIEW_2026-08-15_REBAL1_STAGE2.md`. **Owner decision:** Stage 1's review showed the approved sleeve profile was unreachable under the active policy (90% invested vs a 50% total-exposure cap; growth 40% vs 30% capacity at `max_position_pct` 5%). The owner chose to raise the POLICY rather than lower the profile, for a small testing account. The DEVELOPMENT `assistant/my_policy.json` now carries 0.90/0.07; it is untracked, so no commit contains it, and `assistant/default_policy.json` stays conservative. **The operational checkout was deliberately not touched:** `_active_runtime_lineage` computes the policy fingerprint from the live file and capture refuses on a lineage mismatch, so editing it during the 60-day hold would stall epoch-005 exactly as epoch-002 stalled. **REBAL1CR-002 also fixed:** the Stage 1 counter-review corrected `policy_conflict` masking the band status but left the identical defect on `unassigned_holdings`, so a residual outside its band went uncounted in the headline; `SleeveRow.band_state` is now computed independently of the display label and the breach count derives from it. Stage 2 itself is buy-only: cash and the residual can never receive money, an overweight sleeve produces nothing at all, money is sized to the LOWER BAND EDGE rather than the target, leftover budget is reported rather than pushed onto another sleeve, an unaffordable or unchosen leg is named rather than silently omitted, and proposals bind to the allocation-profile fingerprint so a profile edit cannot reuse them. No submit-all. Stage 3 (tax-aware trims) remains unstarted and needs separate explicit authorization. Full pinned-venv tree 3,971 passed / 0 failed; 26 steering, 71 rebalance, 16 UI tests; 8 mutations, 8 detected. |
