...
| GR-7 product completeness | **Split into sub-milestones 2026-08-05** (the archived plan's five items are far too large for one reviewed branch; one milestone per branch per CLAUDE.md §3). **GR-7a annual tax reporting** — implemented 2026-08-05, awaiting review. **GR-7b idle-cash/mandate reporting**, **GR-7c performance attribution** — open. **GR-7d rebalance-to-target proposals — BLOCKED ON AN OWNER DECISION** (see below), not on code. The archived plan's "tax-aware sell preview" item was found already substantially shipped: `assistant/proposals.py` surfaces `tax_lot_advisory` (lot-level realized-gain consequences) on risk-reduction proposals. | GR-7a done pending review |
...
| # | Item | State |
|---|---|---|
| GR-7a | **Annual tax reporting export** — realized gains by lot, short/long-term split, wash-sale flags, accountant-readable CSV/JSON, coverage honesty | **Implemented 2026-08-05, awaiting independent review.** Pure reporting layer over the already-tested `assistant/tax_lots.py` ledger; no new policy decision. |
