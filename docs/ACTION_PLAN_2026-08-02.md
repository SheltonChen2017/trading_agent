...
| **CR-W2 dividend/cash-movement handler** — implemented 2026-08-10 on `user/claude/broker-dividend-handler-20260810` (owner-authorized; deadline: the AEP dividend bought before the 08-09 ex-date pays **2026-09-09** as a `DIV` activity that would otherwise stall epoch-003 at ~20 sessions). `sync_broker_activities` now journals plain `DIV` → `record_dividend` (tax classification recorded as unknown, per-share arithmetic verified when supplied) and `JNLC`/`CSD`/`CSW` → `record_cash_transfer` (sign-checked); `INT`, all `DIV*` variants, and `JNLS` stay deliberately fail-closed with documented reasons. Per-row refusals aggregate instead of aborting, and the sync report gained a `by_type` breakdown. Eight new tests + one contract update, four reverse mutations verified. | independent review; then owner-triggered epoch roll to epoch-004 (same runbook sequence) well before 2026-09-09, ideally while the session count is near zero | review + owner |
