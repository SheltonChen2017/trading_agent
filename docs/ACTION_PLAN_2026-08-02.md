...
`84e73af` after PR #230 merged the combined REBAL-1 Stage 3 chain -- the
stage itself, Codex's correction, Claude's counter-review, and the
end-to-end coverage against real journaled fills. All REBAL-1 stages are
complete in development and every earlier feature branch has been deleted;
`main` is the only branch on the remote apart from the current round's.
None of this is deployed: the operational runtime remains frozen at
`752d3b7` in active `paper-epoch-005`. Any deployment would close that epoch
by changing its `code_commit`.
