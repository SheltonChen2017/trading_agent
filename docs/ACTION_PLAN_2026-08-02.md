...
caused by uningested broker CAT fees (defect AP-6 below; fix built on a
branch awaiting review). The 60-session / 30-order mandate minimums are NOT
...
| **Broker-activity ingestion (AP-6 fix)** — implemented 2026-08-10 on `user/claude/broker-activity-ingestion-20260810`: paginated raw-REST `list_account_activities()` in `execution/alpaca_broker.py` (pinned alpaca-py lacks the endpoint), fail-closed `sync_broker_activities()` in `assistant/portfolio_ledger.py` (fees journaled idempotently on the broker activity id, `created_at` cutoff at the ledger bootstrap so pre-opening fees are not double-posted, every other activity type refuses loudly), wired into `paper-observation`, `operations-cycle`, and `ledger-reconcile`. Mutation-checked regression tests included. | independent review; then owner decides when to deploy to the operational checkout — **deployment closes `paper-epoch-002`** (1 session, 0 orders, 0 drills recorded — little to lose) and starts epoch-003 | review + owner |
