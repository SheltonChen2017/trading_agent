...
| QC-2 research-look registry | honest denominator for the multiplicity correction on the interactive research surface | **IMPLEMENTED 2026-08-11** on `user/claude/qc2-look-counting-registry-20260811` (awaiting review). **Scope was undefined before this** — the only prior trace was one sentence in the QC-1 row ('Look-counting registry is the next QC milestone') with no definition of done, so the scope below was proposed and owner-approved rather than inherited. New `research_looks` table (new table; migrates fresh and pre-existing databases identically, tested both ways) and `assistant/research_looks.py`. The Backtest page records a look **before** the engine runs — so a configuration cannot be dropped from the denominator once its answer disappoints — and displays the Bonferroni-corrected threshold beside the result. Re-running an identical configuration bumps a repeat counter rather than adding a test (the engine is deterministic, so counting it twice would make the threshold unfairly strict); changing any parameter, or switching synthetic↔real data, is a new look. There is deliberately no delete or configuration update: discarding disappointing looks is the behaviour the correction exists to price in. Recording never gates research — a registry failure warns and the backtest still runs. `assistant/research_looks.py` imports `bonferroni_threshold` from `backtest/engine.py` (already the shared home; `ml/experiments.py` imports it from there too), and `backtest/interactive.py` is untouched so its no-storage AST boundary still holds. 13 tests, 3 mutations verified. **Not a significance test and not a gate** — clearing the threshold is necessary, never sufficient. | independent review | review |
