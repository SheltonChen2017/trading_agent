...
| TRADE-1 | feature | **Owner request 2026-08-14: separate budget/policy trading from owner-directed trading, and support dollar-sized orders.** "Buying" was budget-driven (one amount split across a cart by inverse volatility) and "Selling" was policy-driven (act only on a computed breach). The owner asked to rename those to **Budgeted Buying** and **Policy Based Selling**, and to add **Discrete Buying** and **Discrete Selling** for single-name decisions, each sized either by share count or by dollar amount, with the BUY-1 suggestion picker available on Discrete Buying. | **COMPLETE AFTER INDEPENDENT CORRECTION 2026-08-14; not merged or deployed.** Claude implementation `c1dec52` plus follow-up `c638bc7`; Codex corrections `93953ef` and `7ad7f7d` on `codex/review-trade1-discrete-tabs-20260814`; full disposition and ledger in `docs/REVIEW_2026-08-14_TRADE1_DISCRETE_TRADING.md`. The reviewed feature floors dollar budgets to whole shares with exact Decimal arithmetic, states unspent cash, refuses over-holding sells without silently capping, keeps owner-directed selling separate from breach-based selling, and leaves typed approval plus fresh execution validation unchanged. Claude's follow-up restored the exact fractional-remainder copy and moved SELL-1 tests while review closed the remaining broken suggestion selection, invalid-size stale cards, rounded sell-price boundary, legacy navigation reset, missing suggestion freshness/omission disclosure, Decimal overflow, and direct-generator/UI consistency gaps. The Alpaca-inspired shell now gives all routes one page hierarchy and current Streamlit controls. A full corrected-tree run passed 3,691 tests with 25 warnings; after the moving-head copy reconciliation, the exact final focused suite passed 83 tests and a second full attempt's 15 unrelated failures were isolated to Windows path length and an Aug-14 pre-session date fixture, as detailed in the review. Development remains separate from the frozen epoch-005 runtime at `752d3b7`. |
| SET-1 | feature | **Owner request 2026-08-14: make whole-share-only ordering and the cash reserve owner-configurable.** The owner intends fractional shares for a future automated tool, and runs Alpaca as a small slice of their assets, so an in-account cash reserve is not meaningful to them. Both must stay safe by default. | **IMPLEMENTED on `user/claude/settings-toggles-20260814`; PENDING INDEPENDENT REVIEW; not merged, not deployed.** New policy field `whole_shares_only` (default **True**), plus `risk.execution_gate.is_valid_order_quantity(value, *, whole_shares_only=True)` whose STRICT default is repeated at every call site rather than centralised, so a caller that forgets to thread the flag refuses instead of admitting a fractional order. **`float` stays rejected in BOTH modes** -- permitting fractions is not a licence to reintroduce binary floating point into an order quantity, which is exactly the SELREV-001/002 defect class found twice on 2026-08-13; fractional callers must present an exact `Decimal` or its string form. The cash-reserve toggle deliberately adds **no** new field: `min_cash_reserve_pct` already accepts 0, and one rule with two sources of truth eventually disagrees with itself; unchecking writes 0 and the UI states plainly that this removes the BUFFER, not the solvency check (an order that would take cash negative is still refused, pinned by test). Both toggles sit inside the EXISTING protected policy workflow, so they inherit the typed `UPDATE POLICY` confirmation, atomic save, and expected-fingerprint check. **Deployment consequence:** `compute_policy_fingerprint` hashes every field, so merely adding `whole_shares_only` changes the fingerprint at its safe default -- deploying this closes the active epoch. That is correct (a change to what the system may execute is a material lineage change) and the owner accepted it on 2026-08-14. 29 tests; default-strict, float-still-rejected, and fingerprint-binding each mutation-verified. **Scope: the toggle and its authority only** -- the order path is not yet fractional end to end, and every other layer still refuses fractions. |
