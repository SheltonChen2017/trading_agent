...
| GR-7 product completeness | **Split into sub-milestones 2026-08-05** (the archived plan's five items are far too large for one reviewed branch; one milestone per branch per CLAUDE.md §3). **GR-7a annual tax reporting** — **COMPLETE AND INDEPENDENTLY REVIEWED AFTER CORRECTION 2026-08-05.** Review closed sample-as-broker coverage verification, float-product money conversion, stdout artifact pollution, Reports-page provider-fetch writes, year/report desync, and coverage freeze/outage honesty. **GR-7b idle-cash/mandate reporting** — **COMPLETE AND INDEPENDENTLY REVIEWED AFTER CORRECTION 2026-08-06.** Review closed CLI/UI provider-fetch writes on a claimed read-only surface, NaN measured-vol traceback, and negative measured vol. **GR-7c performance attribution** — **COMPLETE AND INDEPENDENTLY REVIEWED AFTER CORRECTION 2026-08-06.** Single-bucket SPY cash-drag / residual; session sufficiency; review closed silent cash>equity clamp, NaN cost typing, read-only proof, and overinvested label honesty. **GR-7d rebalance-to-target proposals — OWNER DECISION MADE 2026-08-06, now split** (see below). **GR-7d-report** (drift measurement + read-only CLI) is implemented and awaiting independent review; **GR-7d-propose** is not started and is blocked on a newly surfaced conflict, not on the original decision. The archived plan's "tax-aware sell preview" item was found already substantially shipped: `assistant/proposals.py` surfaces `tax_lot_advisory` (lot-level realized-gain consequences) on risk-reduction proposals. | GR-7a/b/c complete after review |
...
| GR-7c | **Performance attribution** — decompose return into allocation/selection/timing/cost/tax rather than the aggregate `performance.py` already reports | **COMPLETE AFTER INDEPENDENT REVIEW 2026-08-06.** `assistant/attribution.py` + CLI `attribution`. Single SPY bucket; sector allocation undefined without mandate weights; selection is a labelled residual; cost/tax already-inside, never re-deducted; session-based sufficiency. |
| GR-7d | **Rebalance-to-target proposals** (+ the `docs/reference/ALLOCATION_SERVICE_DESIGN.md` fold-in) | **OWNER DECISION MADE 2026-08-06 — unblocked and split.** Target: equal weight across all 104 `UNIVERSE` tickers scaled to the policy exposure ceiling, ±25% relative band (inclusive), both directions. Recorded in `docs/OPERATIONAL_FACTS.md` §1. **GR-7d-report (report-only slice) IMPLEMENTED, awaiting independent review**: `assistant/rebalance.py` + read-only CLI `rebalance-report`; measures drift, computes no share counts, proposes nothing. **GR-7d-propose (proposal generation) NOT STARTED** and blocked behind a real conflict: NVDL/BBB and the `CONFIGURED_LEVERAGED_PAIRS` SOXX/SOXL holdings are absent from `UNIVERSE`, so the target implies exiting positions another configured component exists to hold. That must be resolved before proposals can be generated. |

**Why GR-7d WAS blocked, and what replaced the blocker (updated
2026-08-06).** The original reasoning below stands and is kept because it
explains why the target could not be derived in code. It has been
**resolved**: the owner chose equal weight across all 104
`config.REBALANCE_TARGET_TICKERS` scaled to the policy exposure ceiling,
±25% relative band (inclusive), both directions, report-only first. The
decision and the three measurements behind it are recorded in
`docs/OPERATIONAL_FACTS.md` §1.

**A different blocker now stands in front of GR-7d-propose**, discovered
while implementing the report: SOXX/SOXL (traded deliberately by
`CONFIGURED_LEVERAGED_PAIRS`), NVDL and BBB are absent from `UNIVERSE`, so
a UNIVERSE-derived target gives them an implied 0% target — "exit
entirely". A proposal generator would therefore fight a deliberately
configured strategy, with the two components proposing opposite trades.
That is an owner decision about scope, not an engineering fix, and it must
be resolved before any proposal-generating slice. Two smaller open options:
the target currently sits exactly at the exposure ceiling (zero headroom,
so ordinary upward drift reads as a `max_total_exposure_pct` breach), and
`allocation_batch.py` is still buy-only framed while the chosen band is
two-directional.

The original blocking analysis, retained:
