...
| **Operator acknowledgement path for refused broker activities** — implemented 2026-08-11 on `user/claude/broker-activity-acknowledgement-20260811` (awaiting review). Closes the structural problem behind CR-W2/CR-W3: until now an unsupported broker activity blocked evidence capture until someone **deployed code**, and deploying closes the epoch — so one surprise activity type cost the entire accumulated run. New `broker_activity_acknowledgements` table (new table, so `CREATE TABLE IF NOT EXISTS` migrates fresh and pre-existing databases identically; tested both ways), `acknowledge_broker_activity()`, and CLI `ledger-activity-review` / `ledger-activity-acknowledge`. **Nothing is classified automatically.** The operator picks the treatment (`fee`, `dividend`, `cash_transfer`, `no_cash_effect`) and writes a rationale; **every amount comes from the broker row**, so an acknowledgement cannot introduce money the broker never reported. `no_cash_effect` is restricted to rows the broker itself reports as zero — it asserts there is nothing to journal, it does not wave money away. The decision is bound to a SHA-256 fingerprint of the exact activity content, so a changed or id-reused row refuses instead of inheriting an old judgement, and the bootstrap cutoff still outranks it so opening-balance activity can never be resurrected. Recording a decision journals nothing by itself: `sync_broker_activities` stays the single posting path, so application is idempotent and survives a restore. | independent review; then it rides the same epoch-004 roll as CR-W2 and AP-7 | review + owner |
