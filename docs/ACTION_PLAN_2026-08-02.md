...
Current development topology (2026-08-14): `origin/main` is `cfed8c8` after
...
| SET-1 | feature | **Owner request 2026-08-14: make whole-share-only ordering and the cash reserve owner-configurable.** The owner wants optional fractional shares and treats Alpaca as only a small slice of total assets, so an in-account reserve may be unnecessary. Both controls must remain safe by default. | **COMPLETE AFTER INDEPENDENT CORRECTION AND COUNTER-REVIEW 2026-08-14.** Claude's implementation reached `main` through PR #213 at `a62aa1a`. Independent correction `89156b7` merged through PR #217 at `ca0cdf0`, which is now `origin/main`. Claude's counter-review conceded SET1R-001 (the toggle granted a permission no production path could honour), verified the strict-by-default threading, the fail-closed `fractionable` flag, and the removal of the reconciliation tolerance, and closed 1 P2 + 3 P3 of its own: the whole-share floor silently hid an unsellable fractional holding (SET1CR-001), the quantity authority bounded precision but not magnitude (SET1CR-002), a broad handler substituted `Decimal("0")` and skipped the fractionable check (SET1CR-003, not reachable through a durable proposal), and `Decimal(intent.shares)` became a bare string conversion once quantities travelled as text (SET1CR-004). See `docs/REVIEW_2026-08-14_SET1_COUNTERREVIEW.md`. One design question is left open for the owner rather than decided by a reviewer: whether strict mode should permit a fractional sell that closes an entire remaining position. The operational deployment remains at `752d3b7`. `whole_shares_only` defaults to `True`, is boolean-validated and fingerprint-bound, and every omitted call remains strict. When explicitly disabled, Budgeted Buying, Discrete Buying, and Discrete Selling size exact quantities up to nine decimal places; proposal JSON, authorization, gate arithmetic, broker submission, and reconciliation preserve exact decimal text. Fresh validation and submission both require the asset to be broker-marked fractionable; whole orders retain the SDK path and fractional orders use exact REST quantity text. Floats remain invalid. The reserve toggle reuses `min_cash_reserve_pct`: off writes 0, removing only the buffer while negative cash still refuses. Both controls remain behind typed `UPDATE POLICY`, atomic expected-fingerprint persistence, and proposal invalidation. Review closed 4 P2 and 4 P3 findings; see `docs/REVIEW_2026-08-14_SET1_SETTINGS_AND_FRACTIONAL_TRADING.md`. **Deployment consequence:** the new fingerprint field changes execution lineage even at its safe default, so deployment closes epoch-005 and still requires separate owner authorization. |
