...
and zero open alerts. At the independent review, epoch-003 still had **zero
observations**: its start/drill commit lineage is verified, but observation
lineage and the scheduled post-close cadence remain unproven until the first
successful capture. The 60-session / 30-order minimums begin accumulating
from that first post-swap capture. The scarcest
...
| **CR-W2 cash-dividend / explicit cash-transfer handler** — **COMPLETE AFTER INDEPENDENT CORRECTION 2026-08-10**: Claude implementation `25a2e7b` plus Codex correction `a6770f7` on `codex/review-broker-dividend-handler-20260810`. The AEP position acquired 2026-08-07 is scheduled for a $0.95/share dividend on **2026-09-10** (official AEP record/ex-date 2026-08-10). `sync_broker_activities` journals USD plain/explicit-CDIV cash dividends as unknown tax classification and explicit CSD deposits / CSW withdrawals; per-share arithmetic, signs, economic dates, currency, subtype, and cross-type broker-ID identity are fail-closed. Review removed automatic JNLC→contributed-capital treatment because Alpaca defines JNLC only as a generic cash journal, not an owner contribution. Stock dividends, substitute payments, JNLC, interest, withholding/return-of-capital/capital-gain variants, and other unsupported activities remain loud refusals. Per-row refusals aggregate and successful types report a `by_type` breakdown. **Counter-review (Claude, 2026-08-10) confirmed all six findings against the submitted tree (11/11 regressions red) and corrected two residual defects inside the correction: DHCR-001 (P2) — economic dates were stamped at UTC midnight, which is the previous evening in New York, misattributing winter cash flows to the previous session's return interval and a New-Year event to the prior tax year; now stamped at market-local midnight using the tax module's own `MARKET_TIMEZONE`. DHCR-002 (P3) — a hand-maintained prefix literal could `KeyError` past the fail-closed refusal handler; the handled set is now derived from it.** | owner may push/merge; deployment requires a separately authorized epoch-004 roll before 2026-09-10. Watch **CR-W3**: the DIV subtype allowlist (`""`/`CDIV` only) is unverified against a real dividend and may over-refuse once, naming the subtype | review + counter-review complete; owner deployment decision |
...
| AP-7 | P3 | **False-positive critical alert from a negative-age race** (measured read-only 2026-08-10 on the live epoch host). The `portfolio_ledger_reconciliation` health check requires `timedelta(0) <= now - reconciliation_at <= limit and matched`. The lower bound is deliberate (FCS-017: a future-dated row must not read as fresh), but `now` is captured at check entry while a **concurrent** scheduled process writes the reconciliation a moment later (OperationsCycle every 10 min, Watchdog/OrderMonitor every 5). The age goes negative and a **matched, zero-mismatch** reconciliation is reported as a failed critical check — the alert literally reads `matched=True, mismatches=0` with `ok: false`, because the detail string never names which conjunct failed. Measured instance: reconciliation stamped 1.087s AFTER the check's own `now`; 1,885 occurrences since 2026-08-05. Conservative direction (false alarm, not missed alarm) and evidence capture is unaffected — `paper-observation` does not run this check — but an open critical alert feeds the `promotion-status` gate and makes `operations-cycle` exit nonzero. | **Not fixed — deployed code inside the active epoch.** Fix candidates: clamp a small negative age to zero, or re-read `now` after the query, and name the failing conjunct in the detail string. Bundle the reviewed fix with the CR-W2 dividend handler so the single epoch-004 roll before 2026-09-10 covers both. Do not acknowledge the alert as resolved beforehand; it re-raises on the next overlap. |
...
operations-cycle verified green, 0 open alerts). This completes the swap and
epoch-start work, but not the first scheduled-cadence proof: at independent
review epoch-003 had zero observations, so evidence accumulation and
observation-lineage consistency begin only after the first successful
post-close capture. **Full-project sweep
