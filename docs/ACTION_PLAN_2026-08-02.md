...
| TRADE-1 | feature | **Owner request 2026-08-14: separate budget/policy trading from owner-directed trading, and support dollar-sized orders.** "Buying" was budget-driven (one amount split across a cart by inverse volatility) and "Selling" was policy-driven (act only on a computed breach). The owner asked to rename those to **Budgeted Buying** and **Policy Based Selling**, and to add **Discrete Buying** and **Discrete Selling** for single-name decisions, each sized either by share count or by dollar amount, with the BUY-1 suggestion picker available on Discrete Buying. | **IMPLEMENTED on `user/claude/discrete-trading-tabs-20260814`; PENDING INDEPENDENT REVIEW; not merged, not deployed.** Owner decided (2026-08-14) that a dollar amount is a **budget floored to whole shares**, never a notional/fractional order: this project submits whole-share orders only, enforced independently by `risk.execution_gate.is_valid_share_quantity`, by `execution/alpaca_broker.py`'s own copy of it, and by the sell generator's floor, and fractional shares would additionally break tax lots and the never-short guard. New `assistant/discrete_trade.py` owns the dollar<->share conversion in exact Decimal (the SELREV-002 lesson: `3 * 0.10` is `0.30000000000000004` in float, which silently buys one share fewer); it creates no proposals, so both tabs inherit the already-reviewed generators. Buys route through a new `generate_discrete_buy_proposal` added to `assistant/allocation_proposals.py` rather than `build_allocation_plan`, because that planner floors in float — correct for a budget split, an off-by-one for an exact share request. Sells reuse SELL-1's `generate_user_directed_sell_proposal`, which **moved** off the policy page so exactly one owner-directed sell path exists. The unspent remainder of a dollar budget is always displayed, and a dollar amount above the holding is refused rather than silently capped (capping would edit the owner's own instruction). Stale-card binding on ticker AND size follows AP-9/SELL-1. 37 new tests (28 sizing, 9 UI); three properties mutation-verified: floor-not-ceil, leftover disclosure, no-silent-cap. |
