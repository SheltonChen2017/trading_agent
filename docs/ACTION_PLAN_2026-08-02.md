...
| STRONGBUY-PORTFOLIO-20260819 | research / portfolio testing | **Owner-requested complete strategy: every stock passing a frozen Strong-Buy rule, inverse-volatility allocation, point-in-time ETF-holdings overlap, and a capped verified same-index leveraged sleeve.** | **DRAFT — ACCEPTED AFTER INDEPENDENT CORRECTION, BUT NOT ADOPTED, FROZEN, SCHEDULED, OR IMPLEMENTED.** Claude's one-commit amendment `5c42bfd` is dispositioned in `docs/Review/REVIEW_2026-08-19_SBP_PLAN_AMENDMENTS.md`; correction `5c3bf45` closes five P2 and two P3 plan defects. The submitted 33.8% “ceiling,” 35–40% return claim, and 0.6%-monthly “power” claim lacked reproducible evidence or correct statistical framing and are withdrawn. Whole-month price refusal is restored; minimum basket is 10 under the 10% cap; exact price inputs are hash-bound; P3/P4 issuer look-through uses the ordinary same-index exposure with correct 95% scaling; P4−P3 remains descriptive; the inferential family is three one-sided cells at 0.05/3. The 10% overlap floor is an owner-visible policy proposal, not a discovered result. Freeze SBP-0 before any admissible price join; any pre-adoption snapshots are calibration-only. No code, QC, broker, scheduler, or deployment action is authorized by this draft. |
