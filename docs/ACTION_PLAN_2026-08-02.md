...
`45faf1c` after PR #229 merged the complete REBAL-1 Stage 2 review chain.
Claude pushed Stage 3 at exact head `bedeea2`; Codex independently accepted it
after correction `ed6879d` on local branch
`codex/review-rebal1-stage3-20260815`. All three REBAL-1 stages are complete
in development; the review branch is not pushed or merged. None of this is
deployed: the operational runtime remains frozen at `752d3b7` in active
...
| REBAL-1 | feature | **Owner request 2026-08-15: wide-band portfolio rebalancing.** | **ALL THREE STAGES COMPLETE IN DEVELOPMENT AFTER INDEPENDENT REVIEW; not deployed.** Stage 1 is the exact read-only drift report, Stage 2 is owner-directed buy-only cash steering, and Stage 3 is separately approved tax-aware trimming. The allocation targets are owner preference, not evidence of edge. See the three REBAL review reports and `docs/REBAL1_MILESTONE_PLAN.md`. |
...
| REBAL-3 | feature | **Stage 3: tax-aware trims of overweight sleeves, explicitly authorized by the owner on 2026-08-15.** | **ACCEPTED AFTER INDEPENDENT CORRECTION 2026-08-15; local-only review branch, not pushed, merged, or deployed.** Claude's pushed exact head is `bedeea2` (`0490d9d` product, `bedeea2` records); Codex correction is `ed6879d` on `codex/review-rebal1-stage3-20260815`. Review closed eight P2 and one P3 findings: the real coverage contract now works, every owner choice starts unset, fractional input stays exact, working sells are positive/gross and visible, proposal identity and durable evidence bind the tax lots, one clock controls holding-period labels, duplicate named lots refuse, and execution revalidates the complete current lot ledger before broker import. Final validation: 243 focused and 4,026 full-suite tests, 25 known warnings, clean compilation/diff. Lot selection remains advisory to Alpaca; every sale still needs separate typed approval and all normal paper-only execution checks. See `docs/REVIEW_2026-08-15_REBAL1_STAGE3.md`. |
