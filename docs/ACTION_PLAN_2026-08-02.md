...
| **Operator acknowledgement path for refused broker activities** — **COMPLETE AFTER INDEPENDENT CORRECTION 2026-08-11**: Claude implementation merged as PR #188 at `24de4f5`; Codex correction `74376e4` on review branch `codex/review-broker-activity-acknowledgement-20260811`. At review completion that branch had no configured upstream; re-verify current topology rather than preserving that publication state as a milestone fact. New `broker_activity_acknowledgements` table, exact-row fingerprint, and account-bound `ledger-activity-review` / `ledger-activity-acknowledge` turn one unsupported post-bootstrap activity into an explicit operator decision rather than a code deploy. Review made preview genuinely read-only through a verified disposable SQLite snapshot; restricted acknowledgement to rows currently refused by sync; made acknowledgement record-only until the next ordinary sync; enforced executed status, supported currency, explicit amounts/signs, explicit zero for `no_cash_effect`, account/bootstrap binding, timezone-aware audit time, immutable cross-type broker ID, and operator/rationale conflict identity. Nothing is classified automatically and the operator never supplies the amount. Changed rows, old rows, wrong accounts, already-handled rows, and conflicting decisions fail closed. Review closed 6 P2 and 2 P3 findings; 3,404/3,405 collected tests passed, with only the unchanged Streamlit-1.60 hook assertion deselected on this machine's Streamlit 1.52.2. | owner-authorized publication/merge of correction if still pending, then it rides the same separately authorized epoch-004 roll as CR-W2 and AP-7 | owner Git + deployment decision |
