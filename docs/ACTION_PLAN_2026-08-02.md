...
`18a3ee5` after PRs #231 and #232 merged both owner-reported rounds from
exercising the development app -- REBAL-3V, target feasibility made legible,
and REBAL-3W, the trim refusal stating the true reason it refused. Every
REBAL-1 stage and both follow-up rounds are now in `main`; all feature
branches created for them have been deleted after confirming their commits
are reachable from `main`. **Neither follow-up round has had independent
review**; they were merged directly. None of this is deployed: the
...
| REBAL-3V | quality | **Owner-reported usability defect while exercising the dev app, 2026-08-15.** The owner could not find the "Target reachable" column, and reported that the drift table has no horizontal scrollbar. Feasibility was the ninth and last column of a nine-column table, so on a normal window the one fact stating whether the sleeve targets can be reached at all was unreadable in the real app -- and the positive case was never stated at all, leaving "reachable" to be inferred from an absent warning. | **IMPLEMENTED 2026-08-15; MERGED INTO `main` BY PR #231; NOT DEPLOYED; independent review still outstanding.** Implemented on `user/claude/rebal-feasibility-visible-20260815`, which was then stacked under REBAL-3W and deleted after confirming containment; its commits `7e9d005` and `a0a657b` are reachable from `main`. Feasibility is now stated in full BELOW the drift table where width cannot hide it: a bordered block naming each affected sleeve and its exact conflict reason when any target is unreachable, and an explicit positive confirmation naming the active policy file when all of them are reachable. The column survives, shortened to "Reachable", so the per-row fact is still adjacent to the row it describes. No computation changed: `policy_conflict_reason`, `band_state`, the breach count, and the disclosures are all untouched, so this alters presentation only. Validation: 24 UI tests (21 before), 3 mutations against the fix and 3 detected -- including one that initially SURVIVED because the first test asserted only that the reason appeared, not that the sleeve was named, which the total-exposure conflict makes load-bearing since it applies to every funded sleeve at once. Full pinned-venv tree recorded in `docs/SESSION_HANDOFF.md`. |
| REBAL-3W | quality | **Owner-reported contradiction, 2026-08-15, found while exercising Stage 3 on a real book.** The page reported "Bands breached: 6" in its headline and, three subheadings lower, "No sleeve is above its upper band". Both cannot be true, and the second was false: cash and the residual were both above their upper bands. `overweight_sleeves()` filters on two independent conditions at once -- above the band AND trimmable -- so an empty result cannot say which one failed, and the UI reported only one of them. | **IMPLEMENTED 2026-08-15; MERGED INTO `main` BY PR #232 AT `18a3ee5`; NOT DEPLOYED; independent review still outstanding.** Implemented on `user/claude/rebal-trim-refusal-accuracy-20260815`, which carried REBAL-3V as well. **The refusal itself was correct and is unchanged** -- cash is not a holding and the residual is the set of positions the profile does not describe, so trimming there is the reading Stage 1 forbids; only the stated REASON was false, and a refusal that misreports why it fired is indistinguishable from a broken feature, which is the exact shape that cost this workflow ST3R-001 and ST3CCR-001. New `untrimmable_overweight_sleeves()` separates the questions and the page names the over-band-but-untrimmable sleeves and why each is excluded. **An existing test had pinned the false message:** `test_the_trim_section_appears_only_when_a_sleeve_is_overweight` forced a book where cash is the only overweight sleeve -- exactly the owner's situation -- and asserted the false sentence appears, so the defect was tested in; replaced with the accurate assertion. Mutation testing then showed the first fix could MOVE the lie rather than remove it (reporting the untrimmable reason for every empty case survived, and would tell an owner whose book is exactly on target that cash is over its band), so an on-target book is pinned too. Validation: 76 focused tests (74 before), 4 mutations and 4 detected; full pinned-venv tree in `docs/SESSION_HANDOFF.md`. See `docs/REVIEW_2026-08-15_REBAL1_TRIM_REFUSAL_ACCURACY.md`. |
