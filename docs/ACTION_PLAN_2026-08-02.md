...
Current development topology (2026-08-14): `main` and `origin/main` are
`17be33b` after PR #223 merged HEDGE-1, on top of PR #222's epoch-stall
counter-review. Codex independently reviewed the merged HEDGE-1 tree on
`codex/review-hedge1-defensive-sleeve-20260814`; correction `46e1248` awaits
owner authorization to push. The review retained the
owner-directed, equal-weight, proposal-only design while making the public
allowlist authoritative, refusing incomplete selected baskets, subtracting
known pending buys, refusing unknown pending exposure, and preserving exact
numeric inputs. None of this is deployed: the operational runtime remains
separately frozen at `752d3b7` in active `paper-epoch-005`. Any deployment of
the changed code would close that epoch by changing its `code_commit`, even
though HEDGE-1 does not change the mandate or policy fingerprint.
...
| REBAL-1 | feature | **Owner request 2026-08-15: wide-band portfolio rebalancing.** The app had no portfolio-level target or drift surface; its only rebalancing was the leveraged-pair strategy. Codex authored the milestone plan and staged it; Claude implemented Stage 1. | **STAGE 1 IMPLEMENTED 2026-08-15; PENDING INDEPENDENT REVIEW; not merged, not deployed.** Plan in `docs/REBAL1_MILESTONE_PLAN.md`. Stage 0 adds a versioned, fingerprinted SLEEVE allocation profile (`assistant/rebalance_profile.py`) whose targets the owner approved on 2026-08-15 (cash 10 / dividend 15 / growth 40 / leveraged 15 / hedge 10 / other 10, band +/-25% relative) after being shown that the configured sleeves cover about a fifth of the book. A cap is not a target, so nothing is derived. Stage 1 adds a READ-ONLY Portfolio Rebalancing page (`assistant/portfolio_rebalance.py`): equity, invested/cash percentages, breached-band count, sleeve chart, and a drift table with target, band, current, pending, projected, dollar gap to target, and one of seven statuses. It emits no shares, sides, proposals, or approvals. **The confirmed ~89% wide-band finding has `relevant_tickers` `["SOXX","SOXL"]` and does NOT transfer to a general portfolio** -- Codex corrected Claude's original framing, and the module, profile notes, page, and a UI test all carry the correction. Band boundaries are inclusive on both edges; a ticker in two sleeves refuses rather than being assigned; one unusable authoritative value refuses the whole report because sleeve weights share an equity denominator; unassigned holdings are always surfaced and never read as a sell signal; pending buys add and sells subtract into the projection only, with unavailable open-order data refusing it. Stages 2 (buy-only cash steering) and 3 (tax-aware trims, separate authorization) are deliberately not started. Full pinned-venv tree 3,916 passed / 0 failed; 50 module and 8 UI tests; 9 mutations, 9 detected. |
