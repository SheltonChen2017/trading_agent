...
Current development topology (2026-08-16): `main` and `origin/main` are
`dbadb12`, after PR #234 merged the prior Codex REBAL-3V/3W review, PR #236
merged Claude's alpha-battery branch, PR #237 merged Codex's independent
review of it, and PR #238 merged Method V2. Codex reviewed each commit in
`f63fe2c..3d58f6b` from that exact pushed head on
`codex/review-alpha-battery-20260816`. Its product/test correction
`124192f` is merged. The submitted alpha artifacts remain
invalidated pending a clean rerun under
`docs/ALPHA_BATTERY_METHOD_V2.md`. Claude's subsequent QuantConnect branch
was pushed at exact remote head `667cbf4` and independently reviewed on
`codex/review-quantconnect-alpha-20260816`; correction `e8eb558` invalidates
all submitted cloud results and repairs the local measurement/analyser path.
A fresh Claude counter-review and QuantConnect rerun from the final pushed
Codex head are required before any cloud statistic can be cited. None of this is deployed: the operational
runtime remains frozen at `752d3b7` in active `paper-epoch-005`.
Any deployment would close that epoch by changing its `code_commit`.

Alpha development has since moved to the long-lived remote
`origin/user/claude/alpha-qc-round-20260816`. Round 1 was corrected and
counter-reviewed through `af045ee`. Claude's Stage 1 push at exact head
`dc63eec` was independently reviewed on
`codex/review-alpha-qc-round2-20260817`; correction `b143c60` closes the
month-end/21-session timing, point-in-time market-factor, matching-benchmark
and strict-analysis gaps. No Stage 1 QC run occurred in Codex's lane. Claude
must counter-review the final pushed Codex head before any Stage 1 cloud run.
