...
`3d58f6b`, after PR #234 merged the prior Codex REBAL-3V/3W review at
`f63fe2c` and PR #236 merged Claude's alpha-battery branch. Codex reviewed
each commit in `f63fe2c..3d58f6b` from that exact pushed head on
`codex/review-alpha-battery-20260816`; product/test correction `124192f` and
the following records commit are local-only and unpushed. The submitted alpha
artifacts are invalidated pending a clean rerun. None of this is deployed: the
