...
| **CR-W2 cash-dividend / explicit cash-transfer handler** — **COMPLETE, MERGED, NOT DEPLOYED**: Claude implementation `25a2e7b`, Codex correction `a6770f7`, Claude counter-review correction `cf9cdc2`, merged as **PR #184 at `0ee3a22`**. The AEP position acquired 2026-08-07 is scheduled for a $0.95/share dividend on **2026-09-10** (official AEP record/ex-date 2026-08-10). The merged handler journals USD plain/explicit-CDIV cash dividends with unknown tax classification and explicit CSD deposits / CSW withdrawals; economic dates use market-local midnight, and amount, sign, currency, subtype, per-share arithmetic, and broker-ID identity fail closed. JNLC, stock/substitute dividends, interest, tax-specific variants, non-USD amounts, and unknowns remain loud refusals. Claude's counter-review correctly found and fixed the UTC-midnight return/tax-year error and a prefix-map `KeyError` escape; the post-merge review accepted both code corrections. | deployment requires a separately authorized epoch-004 roll before 2026-09-10. Watch **CR-W3**: the `""`/`CDIV` allowlist is not yet verified against a real account dividend and may over-refuse once while naming the subtype | merged; owner deployment decision |
| **Operator acknowledgement path for refused broker activities** — **COMPLETE AFTER INDEPENDENT CORRECTION 2026-08-11**: Claude implementation merged as PR #188 at `24de4f5`; Codex correction `74376e4` on review branch `codex/review-broker-activity-acknowledgement-20260811`. At review completion that branch had no configured upstream; re-verify current topology rather than preserving that publication state as a milestone fact. New `broker_activity_acknowledgements` table, exact-row fingerprint, and account-bound `ledger-activity-review` / `ledger-activity-acknowledge` turn one unsupported post-bootstrap activity into an explicit operator decision rather than a code deploy. Review made preview genuinely read-only through a verified disposable SQLite snapshot; restricted acknowledgement to rows currently refused by sync; made acknowledgement record-only until the next ordinary sync; enforced executed status, supported currency, explicit amounts/signs, explicit zero for `no_cash_effect`, account/bootstrap binding, timezone-aware audit time, immutable cross-type broker ID, and operator/rationale conflict identity. Nothing is classified automatically and the operator never supplies the amount. Changed rows, old rows, wrong accounts, already-handled rows, and conflicting decisions fail closed. Review closed 6 P2 and 2 P3 findings; 3,404/3,405 collected tests passed, with only the unchanged Streamlit-1.60 hook assertion deselected on this machine's Streamlit 1.52.2. | owner-authorized publication/merge of correction if still pending, then it rides the same separately authorized epoch-004 roll as CR-W2 and AP-7. **Counter-review (Claude, 2026-08-11) accepted all eight findings — every one red-baselined against the merged tree (14 failed) — and closed BAACR-001 (P2):** the review's account-binding guard covered the two new standalone commands but not `_sync_broker_activities_from_alpaca`, whose three other callers (`ledger-reconcile`, `paper-observation`, `operations-cycle`) journal activities BEFORE `reconcile_snapshot` checks the binding — so mismatched credentials could write another Alpaca account's fees, dividends, and transfers into this append-only journal, on the two paths that run unattended every ten minutes and nightly. The guard now runs at that single choke point, before the fetch. | owner Git + deployment decision |
...
| **Ticker-suggestion disclosure policy (AP-8b)** — **COMPLETE, NOT YET INDEPENDENTLY REVIEWED, NOT DEPLOYED** on `user/claude/ticker-suggestion-disclosure-20260812`. Owner decision 2026-08-12: this surface is disclosure the reader judges, not a shortlist the project vouches for, and it holds no execution authority, so it should stop screening real securities on size, age, or price. New `SUGGESTION_DISCLOSURE_POLICY` is used by all three lanes of `build_recommended_tickers()`. **Identity screening is deliberately retained** — a symbol must still resolve to real market data, be an `EQUITY`, and be listed on a US venue, because the `ai_suggested` lane is LLM-authored and that floor is what stops a hallucinated symbol from rendering as a suggestion. Every threshold no longer enforced is re-emitted as a visible per-row fact by `_eligibility_disclosure()`, so a thin row is still distinguishable from a blue chip; removing the filter without that would have been a downgrade, not a disclosure. Scope is narrow on purpose: the Watchlist similar-stocks surface still uses `DEFAULT_ELIGIBILITY_POLICY`, whose thresholds are unchanged and pinned by test. Live post-fix run: 10 of 10 most-actives shown, 0 dropped, SPCX and PLUG carrying their disclosure notes. | awaiting independent review; no epoch impact |
