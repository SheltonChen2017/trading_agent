...
`60027af` after PR #219 merged Codex's SET-1 counter-review correction and
PR #220 recorded the owner's decision to leave epoch-005 unchanged for 60
days. Claude's read-only epoch stall detector is `6aa7069` on
`user/claude/epoch-stall-detector-20260814`; Codex accepted it after
correction at `4273de6` on `codex/review-epoch-stall-detector-20260814`.
The correction models the measured fixed Windows-task wall clock, makes a
missing active epoch unhealthy, validates the public boundary, and behaviorally
proves SQLite read-only access. This review branch has not been published.
None of this is deployed: the operational runtime remains separately frozen
at `752d3b7` in active `paper-epoch-005`.
...
| STALL-1 | operations | **Owner request 2026-08-14: detect when paper-evidence observations stop accumulating.** The existing capture path fails closed and alerts on an individual refusal, but the owner also needs a direct read-only answer about the active epoch's multi-session cadence. | **COMPLETE AFTER INDEPENDENT CORRECTION 2026-08-14; not merged or deployed.** Claude implementation `6aa7069`; Codex correction `4273de6`; full disposition in `docs/REVIEW_2026-08-14_EPOCH_STALL_DETECTOR.md`. The standalone CLI classifies `NOT_DUE_YET`, `HEALTHY`, `BEHIND`, `STALLED`, and `NO_ACTIVE_EPOCH`, opens SQLite with enforced `mode=ro`, anchors expected sessions to the active epoch, and treats only the two healthy states as exit 0. Review corrected two P2 and five P3 defects, chiefly replacing market-close-derived timing with the measured fixed 16:30 Pacific task wall clock plus configurable remeasurement options, and making no active epoch unhealthy. Full corrected tree: 3,783 passed. This is advisory only: it does not write, repair, restart, schedule, deploy, roll an epoch, or trade. The operational runtime remains `752d3b7` under the owner's 60-day hold. |
| HEDGE-1 | feature | **Owner request 2026-08-14: add hedging.** The app could buy, sell on policy breach, and sell on owner instruction, but had no way to size a DEFENSIVE position against portfolio exposure. Owner selected an inverse/defensive ETF sleeve over options, and proposal-only over observation-only. | **IMPLEMENTED 2026-08-14; PENDING INDEPENDENT REVIEW; not merged, not deployed.** Claude implementation on `user/claude/hedge1-defensive-sleeve-20260814`. `assistant/hedge_sleeve.py` computes one genuinely new number -- the dollar shortfall between the current defensive weight and an owner-set target -- and delegates all share sizing to `build_allocation_plan`, so preview and proposal cannot drift. Instruments are `config.HEDGE_SLEEVE_TICKERS` (SH, BTAL, TLT, GLD), all verified 2026-08-14 as real, liquid, US-listed ETFs with 400/400 sessions of history. The split is EQUAL WEIGHT, deliberately not inverse-volatility, which would starve whichever defensive name moves least. An unreadable value on any selected holding refuses the whole computation and suppresses the displayed percentage, because an understated current weight is exactly the reading that oversizes a purchase. The page never offers a sell and has no submit-all control; each leg takes its own typed approval. **No mandate behavior field changed:** every instrument is a long-only ETF, `permitted_instruments` already allowed that, and `approved_fingerprint` is verified unchanged, so active `paper-epoch-005` is unaffected. Options, futures, and shorting remain out of scope. 43 module tests plus 8 UI tests; 12 mutations run, 12 detected; full pinned-venv tree 3,834 passed / 0 failed. |
