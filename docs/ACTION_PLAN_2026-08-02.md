...
| POST-CLOSURE-20260818 | research/platform | **Owner adopted the post-closure sequence 1-2-4-3 (2026-08-18).** | **COMPLETE THROUGH SHW-4 AND APQ-5; detailed chronology follows.** (1) Hygiene DONE and independently reviewed 2026-08-18 (`docs/Review/REVIEW_2026-08-18_POST_CLOSURE_MAIN.md`, range `66e2723..f40c2c1`): S1R-001 + SHR-001 accepted. (4) Wide-band check DONE, no work needed: `assistant/rebalance_profile.py` already deploys the owner-approved 0.25 band. (2) design REVISED 2026-08-18 after discovering the ML-LR-6 anti-generic-adapter precedent; SHW-1 (contracts+storage) IMPLEMENTED and independently **accepted** on `main` at `f40c2c1` (PR #259) with five P3 (POST-001..005); POST-001 FIXED and merged (PR #263), unblocking SHW-2 per the owner's direction; **SHW-2 runner IMPLEMENTED 2026-08-18** on `user/claude/shw2-overlay-runner-20260818` at `354a233`; independently reviewed 2026-08-19 (`docs/Review/REVIEW_2026-08-19_SHW2_OVERLAY_RUNNER.md`, range `d4c04c4..354a233`): accepted with two P2 (SHW2-001 baseline unpriced members; SHW2-002 multi-month returns stored as `monthly_returns`) and three P3; **SHW-3 and live epoch registration blocked on SHW2-001/002** — DISCHARGED 2026-08-19: both P2s (and SHW2-003/004; SHW2-005's PIT half) FIXED and mutation-verified in the counter-review round (`docs/Review/REVIEW_2026-08-19_SHW2_COUNTERREVIEW.md`; Decimal half declined with recorded rationale); SHW-3 unblocked after independent verification 2026-08-19 (`docs/Review/REVIEW_2026-08-19_SHW2_FIX_VERIFICATION.md`, range `78258af..128aac8`); leftover P3 SHW2-006/007; SHW-3 may be scheduled, not started automatically; **SHW-3 WAS then owner-scheduled and IMPLEMENTED 2026-08-19** on `user/claude/shw3-sufficiency-20260819` (sufficiency subcommand with the section-6 fields; required_observation_count made a frozen registration contract field with no universal default; counts-only report refusing config drift; read-only against the DB; 3 mutation-verified guards), independently reviewed 2026-08-19 (docs/Review/REVIEW_2026-08-19_SHW3_SUFFICIENCY.md, range 553da76..a384be7): accepted, two P3 (SHW3-001 closed-epoch report; SHW3-002 example 24 is not a freeze); SHW-4 gate freeze DONE 2026-08-19 (owner adopted all proposed values; see the FROZEN preregistration), host decided (dedicated `data/shadow_overlay.db`, operational clone after an owner-approved release advance containing SHW-1..3), and the SHW-3 review settled (SHW3-001 fixed, SHW3-002 discharged — `docs/Review/REVIEW_2026-08-19_SHW3_COUNTERREVIEW.md`): **SHW-4 sub-step 1 EXECUTED 2026-08-19: the defensive-carry stream is LIVE** — epoch `overlay-epoch-001` registered against the frozen preregistration (sha `5479d6b6459a…`, commit `3c9105d`, registration hash `39fca6264e29…`) in the dedicated `shadow_overlay.db`; baseline AVAILABLE at 2026-07-31 with all 108 members priced; the 24-month prospective clock is running. Independent review of `a384be7..a6a690c` ACCEPTED 2026-08-19 (`docs/Review/REVIEW_2026-08-19_SHW4_STREAM_START.md`); `origin/main` already contains PR #267 at `f63ba89` (tree identical to `a6a690c`). Epoch roll and overlay-scheduler install remain owner-present and are not authorized by that review. Paper-task pinning finding stands: the operational installer uses `WorkingDirectory` with no commit pin. (3) `docs/research/DEFENSIVE_CARRY_2026-08-18_PREREGISTRATION.md` **FROZEN 2026-08-19 by owner adoption** (all draft placeholders frozen at proposed values — 15% maxDD / 10% ES / 80% upside-capture composite, ≥8 folds, 2/3-fold + block-bootstrap p<0.05 gate, 24 prospective months) before any confirmation computation; the 2026-07-31 probe is counted as discovery look #1; not a reopening of the closed alpha program. Allocation-policy plan/prereg landed on `main` via PR #260 (`bb6898a`); **APQ-1 SCHEDULED by owner decision 2026-08-19** — APQ-0 is COMPLETE. APQ-1 (LEAN algorithm + local tests, no QC) begins serially after SHW-4; APQ-2..5 follow per `docs/reference/ALLOCATION_POLICY_QC_PLAN.md`, one milestone per branch with review between. |
| STAGE2-PEAD-CLOSED-20260819 | research | **Owner decision 2026-08-19: Stage 2 (point-in-time PEAD, `docs/Alpha_Test_Implementation_Plan.md` section 7) is CLOSED, not implemented.** | **CLOSED.** Reasons recorded: it is cross-sectional stock selection inside the A-002 closure; QC almost certainly lacks point-in-time consensus EPS tied to as-of timestamps (the plan's own feasibility probe would very likely return UNAVAILABLE after real implementation effort); and the prior is poor (earnings-persistence ideas were among the eleven null local signals). Reopening follows the A-002 rule: new owner decision, new universe/data source, fresh preregistration. The archived plan section remains as the historical specification. Restored 2026-08-19 by the SHW-4 independent review after merge `039e5cf` dropped this row (SHW4-001). |
| MAX-PROFIT-POLICY-QC-20260819 | research | **Proposed levered-growth QC family (MPQ): frozen TQQQ/SPY/SOXL mixes vs unlevered 100% SPY, same 2022+ window as APQ, one cloud run, outside the closed alpha program. Daily-reset 3x only; SOXL cap 20%. Not stock-picking. Owner revised the mix to leverage 2026-08-19 before any run.** | **ON HOLD by owner decision 2026-08-19 ("put Cursor's two plans on hold for now") — not frozen, not scheduled; counter-reviewed 2026-08-19 with pre-freeze corrections (`docs/Review/REVIEW_2026-08-19_MPQ_HPQ_PLANS_COUNTERREVIEW.md`).** Owner priority recorded instead: the LEV/SBR strong-buy program. Sequencing updated at counter-review: APQ closed (A-003) the same day, so the original "while APQ-4 is in flight" guard is obsolete — do not start MPQ-1 until the owner freezes the preregistration and schedules it; LEV-2 is the currently scheduled QC-code milestone. Disclosed overlap: LEV's L0-vs-SREF descriptives already contain MPQ's G1-vs-G0 question on a longer window. Spec: `docs/reference/MAX_PROFIT_POLICY_QC_PLAN.md` and `docs/research/MAX_PROFIT_POLICY_2026-08-19_PREREGISTRATION.md`. |
| HEDGE-POLICY-QC-20260819 | research | **Proposed static-hedge QC family (HPQ): frozen SPY+SH / SPY+BTAL overlays vs 100% SPY, same 2022+ window, SH cap 20%. Research only — not the HEDGE-1 Streamlit sleeve.** | **ON HOLD by owner decision 2026-08-19 (same decision as MPQ) — not frozen, not scheduled; counter-reviewed 2026-08-19 with pre-freeze corrections (same record as MPQ).** Sequencing updated at counter-review: APQ is closed (A-003); do not start HPQ-1 until the owner freezes the preregistration and schedules it. Spec: `docs/reference/HEDGE_POLICY_QC_PLAN.md` and `docs/research/HEDGE_POLICY_2026-08-19_PREREGISTRATION.md`. Do not change the HEDGE-1 UI from this row. |
| ALLOCATION-POLICY-QC-20260818 | research | **Reviewer-authored proposal merged to main (PR #260): a frozen allocation-policy QC family (P0-P3 ETF mixes, 2022+ window, one cloud run, own 0.05/3 gate, outside the closed alpha program).** | **CLOSED NULL at A-003 on 2026-08-19; the detailed APQ-1..5 chronology follows.** Authoritative sequencing remains the POST-CLOSURE row: APQ-1 (LEAN algorithm + local tests, no QC) begins serially after SHW-4; APQ-2..5 follow per `docs/reference/ALLOCATION_POLICY_QC_PLAN.md`, one milestone per branch with review between. **APQ-1 IMPLEMENTED 2026-08-19** on `user/claude/apq1-allocation-policy-20260819` (frozen four-policy LEAN algorithm, aligned union refusal, bind-time turnover, 24-month floor, no ACTIVE_UNIVERSE); independently **accepted after correction** 2026-08-19 (`docs/Review/REVIEW_2026-08-19_APQ1_ALLOCATION_POLICY.md`, range `01508b1..e2c4a2b`): APQ1-001 (non-finite closes) closed in that review. **APQ-2 analyser next** (no QC) — **APQ-2 IMPLEMENTED 2026-08-19** on `user/claude/apq2-analyser-20260819` (strict typed refusals, fillna(1.0) charging, versus_p0 descriptives, gate-reachability guard, and the excess-mean test family IN with its 0.05/3 gate and both labels); independently **accepted** 2026-08-19 (`docs/Review/REVIEW_2026-08-19_APQ2_ANALYSER.md`, range `92a0077..5364ae6`): reporting decision RATIFIED (optional family stays in the schema). **APQ-3 IMPLEMENTED 2026-08-19** on `user/claude/apq3-driver-hook-20260819` (universe-free `allocation` family: bytes upload unchanged, still hashed and `require_clean`; `_resolve_universe` refuses both pairing mismatches; anchored no-ACTIVE_UNIVERSE guard; universe-free project name; both plan-required reverse mutations red then restored), independently **accepted after correction** 2026-08-19 (`docs/Review/REVIEW_2026-08-19_APQ3_DRIVER_HOOK.md`, range `95a7210..1a63c8c`): APQ3-001 (incomplete overlay-repair command) closed in that review; counter-review 2026-08-19 VERIFIED the review, confirmed APQ3-001's closure live (owner-executed repair; all three overlay tasks Interactive and proven runnable), and closed APQ3-002 repo-wide (three S4U defaults → Interactive with a mutation-verified scan test) — see `docs/Review/REVIEW_2026-08-19_APQ3_COUNTERREVIEW.md`. **The APQ-1..3 review chain is complete; APQ-4 EXECUTED 2026-08-19 under the owner GO** (merge `5694975`; QC project `35377356`, backtest `b9696f67…`): structurally complete first attempt, 54 months, all four policies, ledgered **R-029 UNANALYSED**, run-level look 29 → 30, no statistic observed. **APQ-5 OBSERVED 2026-08-19 under the owner GO (ledger A-003): NULL — 0 of 3 cells pass the 0.05/3 gate (P1 p=0.080, P2 p=0.121, P3 p=0.214), every excess mean negative; R-029 upgraded to VALID in the same commit; THE ALLOCATION-POLICY QC FAMILY IS CLOSED** (one run and one analyser pass, both spent; descriptive drawdown/Sharpe trade recorded without any claim; paper/live use of any weights is a separate owner decision on the Alpaca/REBAL stack). Interpretation limits recorded in the counter-review: the 2022+ window is regime-conditioned in hindsight (descriptive of that tape, not forward evidence), and the optional-test reporting decision is fixed at APQ-2 review per the plan's counter-review note. |
| LEV-SBR-20260819 | research | **Owner strategy 2026-08-19 ("Strong Buy universe → leveraged ETF → threshold exit"), split at the feasibility probe: QC has no point-in-time analyst ratings.** | **BOTH PREREGISTRATIONS FROZEN BY OWNER ADOPTION 2026-08-19 ("as-is").** **LEV-1 IMPLEMENTED 2026-08-19** on `user/claude/lev1-lean-algorithm-20260819` (`research/lean/leveraged_threshold.py`: daily state machine with next-close execution, month-end vs pullback re-entry, seven aligned monthly series, union refusal, roll-forward turnover across refusal gaps, LSALE tax lines, LEV-specific log markers, no ACTIVE_UNIVERSE; 10 tests, 3 reverse mutations red then restored), accepted after correction by the 2026-08-19 Codex audit; LEV-2 remains gated on merge and counter-review. **SBR-1 IMPLEMENTED 2026-08-19** on `user/claude/sbr1-ratings-capture-20260819` (`scripts/capture_analyst_ratings.py` + frozen 102-ticker NASDAQ-100 config with deterministic Wikipedia provenance + Interactive-logon installer at 17:15 ET; per-ticker failures recorded not dropped, market-time month labels, canonical-LF hashed snapshots, orphan/hash-mismatch refusals, no evaluation imports; 15 tests, 3 reverse mutations red then restored), accepted after correction by the same audit; owner-present task install remains gated on merge and counter-review. NO evaluation of snapshots until the evaluation contract is frozen. **SBP (Strong-Buy portfolio test) 2026-08-19: Codex drafted `docs/reference/STRONGBUY_PORTFOLIO_TEST_PLAN.md` — the full strategy test (ratings filter → inverse-vol basket → ETF-overlap selection → capped leveraged sleeve) with P0..P4 isolating one decision at a time. Counter-reviewed and AMENDED the same day (SBPA-001..005) — overlap threshold 50% → 10% after a declared structural feasibility check measured a 33.8% ceiling (50% would have refused the overlay every month); ticker-level rather than whole-month price refusal; P4−P3 demoted to descriptive so the frozen family is 3 cells at 0.05/3; declared power arithmetic (24 months detects only ≈0.6%/month) with the meaning of a null fixed in advance; and SBP-0 supersedes the SBR-2 evaluation step. RECOMMENDED as the PRIMARY plan, with LEV secondary. PENDING OWNER ADOPTION — no SBP code before the freeze.** Path B = LEV family (`docs/research/LEVERAGED_THRESHOLD_2026-08-19_PREREGISTRATION.md`): TQQQ take-profit/re-entry state machine, 4 variants, 8 cells at 0.05/8 (vs TQQQ buy-and-hold = the edge test; vs QQQ = the owner's goal, leverage-labeled), preregistered 37%/20% after-tax descriptive column per the SOXL lesson, one cloud run + one analyser pass. Path C = SBR capture stream (`docs/research/STRONGBUY_RATINGS_2026-08-19_CAPTURE_PREREGISTRATION.md`): monthly point-in-time-by-construction snapshots of NASDAQ-100 analyst consensus (yfinance), task-specific runtime (ML-LR-6 precedent), Interactive-logon scheduled task, NO evaluation look until a separate SBR-2 preregistration freezes after ≥12 snapshots. Both explicitly outside the closed A-002 program (fixed instruments / new data source + fresh preregistration + owner decision). Milestones LEV-1..4 and SBR-1 start only after adoption, one branch + review each. |
| STRONGBUY-PORTFOLIO-20260819 | research / portfolio testing | **Owner-requested revision of the complete Strong-Buy idea: include every stock that passes a frozen Strong-Buy rule, allocate the stock basket by inverse volatility, select an ETF by point-in-time weight-based holdings overlap, and add only a capped verified same-index leveraged sleeve.** | **DRAFT — NOT ADOPTED, NOT FROZEN, NOT SCHEDULED.** Detailed SBP-0..5 proposal: `docs/reference/STRONGBUY_PORTFOLIO_TEST_PLAN.md`. It preserves the frozen SBR-1 capture-only contract and makes the missing evaluation design explicit. P0–P4 isolate ratings selection, inverse-volatility sizing, ordinary ETF overlap, and leverage in separate comparisons; a 5% leveraged sleeve and look-through issuer cap are proposed. The existing LEV family remains a separate generic TQQQ timing study and must not be described as validation of the complete Strong-Buy portfolio. Freeze this successor before any admissible snapshot is joined to subsequent prices; any pre-adoption snapshots are calibration-only. SBR installation, official ETF-pair verification, holdings capture, constructor code, paper activity, QC custom-data replay, and deployment each retain their own review/owner gates. No implementation or operational action is authorized by this draft. |
