...
| GR-7 product completeness | **Split into sub-milestones 2026-08-05** (the archived plan's five items are far too large for one reviewed branch; one milestone per branch per CLAUDE.md §3). **GR-7a annual tax reporting** — **COMPLETE AND INDEPENDENTLY REVIEWED AFTER CORRECTION 2026-08-05.** Review closed sample-as-broker coverage verification, float-product money conversion, stdout artifact pollution, Reports-page provider-fetch writes, year/report desync, and coverage freeze/outage honesty. **GR-7b idle-cash/mandate reporting** — **COMPLETE AND INDEPENDENTLY REVIEWED AFTER CORRECTION 2026-08-06.** Review closed CLI/UI provider-fetch writes on a claimed read-only surface, NaN measured-vol traceback, and negative measured vol. **GR-7c performance attribution** — open. **GR-7d rebalance-to-target proposals — BLOCKED ON AN OWNER DECISION** (see below), not on code. The archived plan's "tax-aware sell preview" item was found already substantially shipped: `assistant/proposals.py` surfaces `tax_lot_advisory` (lot-level realized-gain consequences) on risk-reduction proposals. | GR-7a/b complete after review |
...
| GR-7c | **Performance attribution** — decompose return into allocation/selection/timing/cost/tax rather than the aggregate `performance.py` already reports | **IMPLEMENTED 2026-08-06, AWAITING INDEPENDENT REVIEW.** `assistant/attribution.py` (pure, read-only) + CLI `attribution`. Single-bucket benchmark (**SPY**, matching the ticker `paper_evidence` already binds into every observation — a second index would put two benchmarks in one epoch). Sector allocation is **undefined**, not deferred: the mandate states a volatility band, not target weights, so there is nothing to be over/under-weight against. The one well-defined bucket is invested-vs-cash, making `allocation` = cash drag and `selection` an explicitly labelled RESIDUAL (absorbs leverage and concentration, not a skill measure). Cost/tax are reported as already inside the equity curve, never re-deducted. Weights are beginning-of-period. Sufficiency counts **independent market sessions**, not valuation points. |
