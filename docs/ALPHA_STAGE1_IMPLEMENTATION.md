...
`tests/test_alpha_stage1_replications.py` (committed as `dc63eec`)
Status: **Implemented and validated locally. NOT run on QuantConnect.**
...
Only score formation and the specification list differ from
`alpha_battery_monthly.py`. That file's timing, delisting, session
alignment, basket freezing, drift-aware turnover and completeness
behaviour has been reviewed twice (QCAR-001/002/003/005, AQR1-001/005) and
