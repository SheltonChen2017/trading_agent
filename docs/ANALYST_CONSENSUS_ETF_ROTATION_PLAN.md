...
| Execution simulation, fees, slippage | QuantConnect Cloud (authoritative); local LEAN (development/test only) | **Cloud selected; ACER data path unresolved.** Local LEAN CLI `1.0.228`, Docker `29.7.2`, and a generated sample execution through LEAN Engine `2.5.0.0` passed on 2026-08-21, but that environment is not used for ACER historical/outcome backtests. Cloud coverage, semantics, provenance and the applicable vendor-use/third-party-processing terms must be verified before any ACER outcome run. |
| **Analyst revision history** | Benzinga Analyst Ratings via Massive | **Purchased and structurally audited.** Massive explicitly documents backtesting rating impact as a use case, and its investment-advice disclaimer is not a personal-research ban. The immutable machine-local snapshot remains licensed data and is not committed. Whether the purchase-specific order form and additional terms permit the selected representation to be processed through QuantConnect custom data remains unverified; those terms may already grant it, so a separate permission letter is not presumed necessary. Issuer mapping and Snapshot B remain open ACER-1 gates. |
