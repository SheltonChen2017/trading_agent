...
independent draft is preserved at `docs/Archive/Plans/ACTION_PLAN_codex.md`;
...
Current development topology (2026-08-19): the independent Codex audit
reviews exact pushed range
`81db126340818fe2c2c9efa16c77af8f1d37568f..3055fecd1caf490c852a446c03da760d2878af5a`
(143 commits) on `codex/review-post-stage0-through-sbr1-20260819`.
Stage 0 and Stage 1 are complete and null (A-001/A-002); the
allocation-policy family is complete and null (R-029/A-003); SHW-4 is
live and collecting prospective evidence; LEV-1 and SBR-1 are accepted
only after the audit corrections recorded in that review branch. LEV-2
and the owner-present SBR scheduler install remain gated on merge and
...
cycle. See
`docs/Archive/Review/REVIEW_2026-08-17_ALPHA_QC_FABLE_COUNTERREVIEW.md`. Claude's
...
were removed from active docs only after `docs/Archive/Research/alpha-result.md` preserved
...
`docs/Archive/Review/REVIEW_2026-08-09_AUI_CORRECTIONS.md`. This presentation milestone does
...
`docs/Archive/Review/REVIEW_2026-08-11_MOST_ACTIVE_DIRECTION_SPLIT.md`.
...
| REBAL-3 | feature | **Stage 3: tax-aware trims of overweight sleeves, explicitly authorized by the owner on 2026-08-15.** | **ACCEPTED AFTER INDEPENDENT CORRECTION 2026-08-15; MERGED INTO `main` BY PR #230 AT `84e73af`; NOT DEPLOYED.** Claude's pushed exact head is `bedeea2` (`0490d9d` product, `bedeea2` records); Codex correction is `ed6879d` on `codex/review-rebal1-stage3-20260815`. Review closed eight P2 and one P3 findings: the real coverage contract now works, every owner choice starts unset, fractional input stays exact, working sells are positive/gross and visible, proposal identity and durable evidence bind the tax lots, one clock controls holding-period labels, duplicate named lots refuse, and execution revalidates the complete current lot ledger before broker import. Final validation: 243 focused and 4,026 full-suite tests, 25 known warnings, clean compilation/diff. Lot selection remains advisory to Alpaca; every sale still needs separate typed approval and all normal paper-only execution checks. See `docs/Archive/Review/REVIEW_2026-08-15_REBAL1_STAGE3.md`. |
- **Counter-review (Claude, 2026-08-15): all nine review findings independently re-derived on a worktree at the submitted tree `bedeea2`; all nine confirmed.** ST3R-001 is the headline and deserves stating plainly: the submitted feature REFUSED EVERY TRIM, always, because it read a per-ticker `complete` key the real coverage provider never emits (`broker_shares`/`ledger_shares`/`matched`). The tests passed only because the fixture was an invented shape rather than one obtained from the real producer -- the same root cause as Stage 2's REBAL2CR-001 one round earlier, and worse here because a refusal that always fires is indistinguishable from a careful safeguard. ST3R-002 also contradicted the submitted documentation: the sleeve picker auto-selected while the handoff claimed all four owner decisions start unset. ST3R-007 was a defect in shared tax machinery -- `select_lots` took 150 shares from a 100-share lot on a duplicated lot id. **One P2 closed at `user/claude/rebal1-stage3-counterreview-20260815` (ST3CCR-001):** the corrected coverage gate still required GLOBAL completeness, and since `list_fills` documents that pre-app holdings produce no lots, a single such holding refused every trim permanently -- the same always-refuses outcome through a different gate, and the owner's real book would have hit it. Both the creation and approval gates are now scoped to the trimmed ticker's `matched` flag, which is necessary and sufficient because the sale realizes gains from that ticker's lots alone; the uncovered remainder is disclosed instead of blocking. The ST3R-008 execution-path change was audited separately and accepted. See `docs/Archive/Review/REVIEW_2026-08-15_REBAL1_STAGE3_COUNTERREVIEW.md`. Full pinned-venv tree 4,031 passed / 0 failed; 4 mutations against the fix, all detected. |
| REBAL-3E | quality | **Owner request 2026-08-15: add the end-to-end test with real fills.** Three consecutive rounds of Stage 2 and Stage 3 failed the same way -- an interface-shape mistake that hand-written fixtures could not detect -- and two of them produced a refusal that always fired, which reads exactly like a careful safeguard. | **IMPLEMENTED 2026-08-15; MERGED INTO `main` BY PR #230 AT `84e73af`; NOT DEPLOYED; still awaiting independent review of the end-to-end round itself.** Branch `user/claude/rebal1-e2e-real-fills-20260815`; disposition in `docs/Archive/Review/REVIEW_2026-08-15_REBAL1_STAGE3_END_TO_END.md`. `tests/test_rebalance_trim_end_to_end.py` invents no shapes: fills are journaled through `journal_broker_order_update`, ledger and coverage come from the real providers, the proposal is persisted through `AssistantStore.save_proposal` and reloaded, and approval runs the real validation path. **Writing it immediately found a fourth instance of the same defect, in my own previous fix:** `tax_ledger_with_coverage` returns `(ledger if complete else None, ...)`, so it withholds the ledger ENTIRELY when any holding is unreconciled -- meaning the ST3CCR-001 fix, verified only against a hand-built dict pairing a real ledger with `complete: False`, never worked against the real provider and Stage 3 was still unusable on any book with a pre-app holding. Fixed with a new sibling `ticker_tax_ledger_with_coverage(store, portfolio, ticker)` rather than by loosening the shared portfolio-wide contract: the two answer different questions, and a trim's realized gain depends on one ticker's lots alone. All three call sites -- `plan_trim`, the execution-time revalidation, and the Stage 3 UI -- now use it. Remaining gap recorded: no test clicks the Streamlit trim button through to a saved proposal. Full pinned-venv tree 4,041 passed / 0 failed; 10 new end-to-end tests; 3 mutations against the new provider, 3 detected. |
| REBAL-3V | quality | **Owner-reported usability defect while exercising the dev app, 2026-08-15.** The owner could not find the "Target reachable" column, and reported that the drift table has no horizontal scrollbar. Feasibility was the ninth and last column of a nine-column table, so on a normal window the one fact stating whether the sleeve targets can be reached at all was unreadable in the real app -- and the positive case was never stated at all, leaving "reachable" to be inferred from an absent warning. | **ACCEPTED AFTER INDEPENDENT CORRECTION; merged by PR #231, with correction `3a506ae` and records `dae34d0` merged by PR #234 at `f63fe2c`; not deployed.** The width-independent positive/negative block and per-row "Reachable" value are correct and presentation-only. Review closed one P3 test-sensitivity defect: one submitted pytest was empty, and the claimed positive-case test accepted either exclusive branch while depending on an ignored personal policy. Deterministic coverage pins the positive case plus all four real conflict rules. See `docs/Archive/Review/REVIEW_2026-08-15_REBAL3V_REBAL3W_INDEPENDENT.md`. |
| REBAL-3W | quality | **Owner-reported contradiction, 2026-08-15, found while exercising Stage 3 on a real book.** The page reported "Bands breached: 6" in its headline and, three subheadings lower, "No sleeve is above its upper band". Both cannot be true, and the second was false: cash and the residual were both above their upper bands. The submitted generic `overweight_sleeves()` combined above-band and trimmable into one filtered list, so an empty result lost which condition failed. | **ACCEPTED AFTER INDEPENDENT CORRECTION; merged by PR #232 at `18a3ee5`, with correction `3a506ae` and records `dae34d0` merged by PR #234 at `f63fe2c`; not deployed.** The refusal/eligibility direction is unchanged. One immutable classification now returns both trimmable and untrimmable groups, and the refusal states the exact trim-eligible fact. See `docs/Archive/Review/REVIEW_2026-08-15_REBAL3V_REBAL3W_INDEPENDENT.md`. |
| ALPHA-BATTERY-20260815/16 | research | **Owner-requested broad alpha battery and three-universe rerun, merged by PR #236 at `3d58f6b`.** | **SUBMITTED RESULTS INVALIDATED BY INDEPENDENT REVIEW 2026-08-16; no signal confirmed.** Review of every commit in `f63fe2c..3d58f6b` closed five P2 and two P3 findings in correction `124192f` (merged by PR #237): the first test could not attain its own Bonferroni threshold; long/short side flips registered zero turnover; market caps mixed split-adjusted prices with raw shares; SEC availability was guessed instead of filed; the “survivorship loss” denominator preceded the universe screens; the latest size bucket leaked into all historical industry/residual scores; and the merged classifier called near-zero broad results robust. Invalid generated JSON/Markdown was removed from active docs on 2026-08-17 after its hashes and disposition were preserved in `docs/Archive/Research/alpha-result.md`. Corrected code uses actual filing dates and raw screening prices, refuses old caches and unavailable point-in-time industry inputs, and labels the panel non-point-in-time. A clean reviewed QC rerun is required; no registry, mandate, proposal, execution, or epoch authority changed. See `docs/Archive/Review/REVIEW_2026-08-16_ALPHA_BATTERY.md`. |
| QC-ALPHA-20260816 | research | **Claude's QuantConnect smoke work and A/B/C replication battery at pushed head `667cbf4`.** | **ALL SUBMITTED CLOUD RESULTS REJECTED; FULL RE-AUDIT SUPERSEDES THE EARLIER CORRECTION.** The first independent review closed ten P2 and one P3 findings in `e8eb558`; the 2026-08-17 full-tree audit found additional current-API, framework-shadowing, PIT-factor, exact-session, provenance, numeric-refusal, and stall-handling defects and corrected them in `855941a`. Invalid generated results/logs were removed from active docs after permanent ledger preservation. No QuantConnect call occurred during either Codex review. Claude must counter-review and rerun exact final pushed source before any result is cited. See `docs/Archive/Review/REVIEW_2026-08-16_QUANTCONNECT_ALPHA_BATTERY.md` and the round-2 report. |
| ALPHA-QC-ROUND1-20260816 | research | **First staged Claude push on `origin/user/claude/alpha-qc-round-20260816` at exact head `ad6475d`: five preserved QC artifacts, result-ledger opening, and residual peer-length fix.** | **ACCEPTED AFTER CORRECTION; COUNTER-REVIEW AND NEW QC RERUN PENDING.** Independent review closed four P2 and one P3 findings. The peer equality was a real refusal cause, but the slice-only fix measured the skipped latest month instead of the 6-1/12-1 formation window. Product correction `8bf8a82` now uses a fixed 252-session joint market/leave-one-out-industry fit, measures 105/231 formation sessions, skips the latest 21, and has behavioral tests. Follow-up `56bc86d` binds every deque close to its exact exchange session and refuses missing, duplicated, or post-universe-gap history instead of manufacturing adjacent returns. The ledger now counts all five cloud executions, records 80 emitted repeated-look cells and a conservative lifetime floor of 428, acknowledges the existing base64 decoder, uses exact artifact hashes/backtest IDs, and marks missing compile/project identities. No saved market output was analysed and no QC access occurred. `docs/Archive/Plans/Alpha_Test_Implementation_Plan.md` freezes the staged replication/new-hypothesis program and reserves Alpaca Paper for later forward/execution testing. See `docs/Archive/Review/REVIEW_2026-08-16_ALPHA_QC_ROUND1.md`. |
| ALPHA-QC-STAGE1-20260817 | research | **Claude's REP-H52 / REP-IDV Stage 1 implementation at exact pushed head `dc63eec`.** | **ACCEPTED AFTER CORRECTION; SUPERSEDED 2026-08-18 — QC EXECUTION COMPLETE AND NULL, PROGRAM CLOSED (ledger A-002; closes review finding S1R-003; the sentence that follows is the historical 2026-08-17 record).** Original status: CLAUDE COUNTER-REVIEW AND QC EXECUTION PENDING. Independent review closed two P1 and two P2 findings in `b143c60`. Submitted scoring happened at the first monthly session and settlement at the next month's entry, not the frozen month-end/next-close/exact-21 experiment; REP-IDV applied the score-date universe backward across 111 factor days and filled unavailable factor returns with zero; and no cadence-matched benchmark or Stage 1 analyser existed. Corrected code freezes the immediately preceding month-end, enters next close, tracks overlapping exact-21-session cohorts, records PIT daily equal-weight factor returns with exact-session refusal, adds a matching benchmark, and requires full alpha/benchmark run identity plus 24-cell and 452-cell gates. No QC access or run occurred and the lifetime floor remains 428. See `docs/Archive/Review/REVIEW_2026-08-17_ALPHA_QC_ROUND2.md`. |
| POST-CLOSURE-20260818 | research/platform | **Owner adopted the post-closure sequence 1-2-4-3 (2026-08-18).** | **COMPLETE THROUGH SHW-4 AND APQ-5; detailed chronology follows.** (1) Hygiene DONE and independently reviewed 2026-08-18 (`docs/Archive/Review/REVIEW_2026-08-18_POST_CLOSURE_MAIN.md`, range `66e2723..f40c2c1`): S1R-001 + SHR-001 accepted. (4) Wide-band check DONE, no work needed: `assistant/rebalance_profile.py` already deploys the owner-approved 0.25 band. (2) design REVISED 2026-08-18 after discovering the ML-LR-6 anti-generic-adapter precedent; SHW-1 (contracts+storage) IMPLEMENTED and independently **accepted** on `main` at `f40c2c1` (PR #259) with five P3 (POST-001..005); POST-001 FIXED and merged (PR #263), unblocking SHW-2 per the owner's direction; **SHW-2 runner IMPLEMENTED 2026-08-18** on `user/claude/shw2-overlay-runner-20260818` at `354a233`; independently reviewed 2026-08-19 (`docs/Archive/Review/REVIEW_2026-08-19_SHW2_OVERLAY_RUNNER.md`, range `d4c04c4..354a233`): accepted with two P2 (SHW2-001 baseline unpriced members; SHW2-002 multi-month returns stored as `monthly_returns`) and three P3; **SHW-3 and live epoch registration blocked on SHW2-001/002** — DISCHARGED 2026-08-19: both P2s (and SHW2-003/004; SHW2-005's PIT half) FIXED and mutation-verified in the counter-review round (`docs/Archive/Review/REVIEW_2026-08-19_SHW2_COUNTERREVIEW.md`; Decimal half declined with recorded rationale); SHW-3 unblocked after independent verification 2026-08-19 (`docs/Archive/Review/REVIEW_2026-08-19_SHW2_FIX_VERIFICATION.md`, range `78258af..128aac8`); leftover P3 SHW2-006/007; SHW-3 may be scheduled, not started automatically; **SHW-3 WAS then owner-scheduled and IMPLEMENTED 2026-08-19** on `user/claude/shw3-sufficiency-20260819` (sufficiency subcommand with the section-6 fields; required_observation_count made a frozen registration contract field with no universal default; counts-only report refusing config drift; read-only against the DB; 3 mutation-verified guards), independently reviewed 2026-08-19 (docs/Archive/Review/REVIEW_2026-08-19_SHW3_SUFFICIENCY.md, range 553da76..a384be7): accepted, two P3 (SHW3-001 closed-epoch report; SHW3-002 example 24 is not a freeze); SHW-4 gate freeze DONE 2026-08-19 (owner adopted all proposed values; see the FROZEN preregistration), host decided (dedicated `data/shadow_overlay.db`, operational clone after an owner-approved release advance containing SHW-1..3), and the SHW-3 review settled (SHW3-001 fixed, SHW3-002 discharged — `docs/Archive/Review/REVIEW_2026-08-19_SHW3_COUNTERREVIEW.md`): **SHW-4 sub-step 1 EXECUTED 2026-08-19: the defensive-carry stream is LIVE** — epoch `overlay-epoch-001` registered against the frozen preregistration (sha `5479d6b6459a…`, commit `3c9105d`, registration hash `39fca6264e29…`) in the dedicated `shadow_overlay.db`; baseline AVAILABLE at 2026-07-31 with all 108 members priced; the 24-month prospective clock is running. Independent review of `a384be7..a6a690c` ACCEPTED 2026-08-19 (`docs/Archive/Review/REVIEW_2026-08-19_SHW4_STREAM_START.md`); `origin/main` already contains PR #267 at `f63ba89` (tree identical to `a6a690c`). Epoch roll and overlay-scheduler install remain owner-present and are not authorized by that review. Paper-task pinning finding stands: the operational installer uses `WorkingDirectory` with no commit pin. (3) `docs/research/DEFENSIVE_CARRY_2026-08-18_PREREGISTRATION.md` **FROZEN 2026-08-19 by owner adoption** (all draft placeholders frozen at proposed values — 15% maxDD / 10% ES / 80% upside-capture composite, ≥8 folds, 2/3-fold + block-bootstrap p<0.05 gate, 24 prospective months) before any confirmation computation; the 2026-07-31 probe is counted as discovery look #1; not a reopening of the closed alpha program. Allocation-policy plan/prereg landed on `main` via PR #260 (`bb6898a`); **APQ-1 SCHEDULED by owner decision 2026-08-19** — APQ-0 is COMPLETE. APQ-1 (LEAN algorithm + local tests, no QC) begins serially after SHW-4; APQ-2..5 follow per `docs/Archive/Plans/ALLOCATION_POLICY_QC_PLAN.md`, one milestone per branch with review between. |
| STAGE2-PEAD-CLOSED-20260819 | research | **Owner decision 2026-08-19: Stage 2 (point-in-time PEAD, `docs/Archive/Plans/Alpha_Test_Implementation_Plan.md` section 7) is CLOSED, not implemented.** | **CLOSED.** Reasons recorded: it is cross-sectional stock selection inside the A-002 closure; QC almost certainly lacks point-in-time consensus EPS tied to as-of timestamps (the plan's own feasibility probe would very likely return UNAVAILABLE after real implementation effort); and the prior is poor (earnings-persistence ideas were among the eleven null local signals). Reopening follows the A-002 rule: new owner decision, new universe/data source, fresh preregistration. The archived plan section remains as the historical specification. Restored 2026-08-19 by the SHW-4 independent review after merge `039e5cf` dropped this row (SHW4-001). |
| MAX-PROFIT-POLICY-QC-20260819 | research | **Proposed levered-growth QC family (MPQ): frozen TQQQ/SPY/SOXL mixes vs unlevered 100% SPY, same 2022+ window as APQ, one cloud run, outside the closed alpha program. Daily-reset 3x only; SOXL cap 20%. Not stock-picking. Owner revised the mix to leverage 2026-08-19 before any run.** | **ON HOLD by owner decision 2026-08-19 ("put Cursor's two plans on hold for now") — not frozen, not scheduled; counter-reviewed 2026-08-19 with pre-freeze corrections (`docs/Archive/Review/REVIEW_2026-08-19_MPQ_HPQ_PLANS_COUNTERREVIEW.md`).** Owner priority recorded instead: the LEV/SBR strong-buy program. Sequencing updated at counter-review: APQ closed (A-003) the same day, so the original "while APQ-4 is in flight" guard is obsolete — do not start MPQ-1 until the owner freezes the preregistration and schedules it; LEV-2 is the currently scheduled QC-code milestone. Disclosed overlap: LEV's L0-vs-SREF descriptives already contain MPQ's G1-vs-G0 question on a longer window. Spec: `docs/Plan/MAX_PROFIT_POLICY_QC_PLAN.md` and `docs/Plan/Research/MAX_PROFIT_POLICY_2026-08-19_PREREGISTRATION.md`. |
| HEDGE-POLICY-QC-20260819 | research | **Proposed static-hedge QC family (HPQ): frozen SPY+SH / SPY+BTAL overlays vs 100% SPY, same 2022+ window, SH cap 20%. Research only — not the HEDGE-1 Streamlit sleeve.** | **ON HOLD by owner decision 2026-08-19 (same decision as MPQ) — not frozen, not scheduled; counter-reviewed 2026-08-19 with pre-freeze corrections (same record as MPQ).** Sequencing updated at counter-review: APQ is closed (A-003); do not start HPQ-1 until the owner freezes the preregistration and schedules it. Spec: `docs/Plan/HEDGE_POLICY_QC_PLAN.md` and `docs/Plan/Research/HEDGE_POLICY_2026-08-19_PREREGISTRATION.md`. Do not change the HEDGE-1 UI from this row. |
| ALLOCATION-POLICY-QC-20260818 | research | **Reviewer-authored proposal merged to main (PR #260): a frozen allocation-policy QC family (P0-P3 ETF mixes, 2022+ window, one cloud run, own 0.05/3 gate, outside the closed alpha program).** | **CLOSED NULL at A-003 on 2026-08-19; the detailed APQ-1..5 chronology follows.** Authoritative sequencing remains the POST-CLOSURE row: APQ-1 (LEAN algorithm + local tests, no QC) begins serially after SHW-4; APQ-2..5 follow per `docs/Archive/Plans/ALLOCATION_POLICY_QC_PLAN.md`, one milestone per branch with review between. **APQ-1 IMPLEMENTED 2026-08-19** on `user/claude/apq1-allocation-policy-20260819` (frozen four-policy LEAN algorithm, aligned union refusal, bind-time turnover, 24-month floor, no ACTIVE_UNIVERSE); independently **accepted after correction** 2026-08-19 (`docs/Archive/Review/REVIEW_2026-08-19_APQ1_ALLOCATION_POLICY.md`, range `01508b1..e2c4a2b`): APQ1-001 (non-finite closes) closed in that review. **APQ-2 analyser next** (no QC) — **APQ-2 IMPLEMENTED 2026-08-19** on `user/claude/apq2-analyser-20260819` (strict typed refusals, fillna(1.0) charging, versus_p0 descriptives, gate-reachability guard, and the excess-mean test family IN with its 0.05/3 gate and both labels); independently **accepted** 2026-08-19 (`docs/Archive/Review/REVIEW_2026-08-19_APQ2_ANALYSER.md`, range `92a0077..5364ae6`): reporting decision RATIFIED (optional family stays in the schema). **APQ-3 IMPLEMENTED 2026-08-19** on `user/claude/apq3-driver-hook-20260819` (universe-free `allocation` family: bytes upload unchanged, still hashed and `require_clean`; `_resolve_universe` refuses both pairing mismatches; anchored no-ACTIVE_UNIVERSE guard; universe-free project name; both plan-required reverse mutations red then restored), independently **accepted after correction** 2026-08-19 (`docs/Archive/Review/REVIEW_2026-08-19_APQ3_DRIVER_HOOK.md`, range `95a7210..1a63c8c`): APQ3-001 (incomplete overlay-repair command) closed in that review; counter-review 2026-08-19 VERIFIED the review, confirmed APQ3-001's closure live (owner-executed repair; all three overlay tasks Interactive and proven runnable), and closed APQ3-002 repo-wide (three S4U defaults → Interactive with a mutation-verified scan test) — see `docs/Archive/Review/REVIEW_2026-08-19_APQ3_COUNTERREVIEW.md`. **The APQ-1..3 review chain is complete; APQ-4 EXECUTED 2026-08-19 under the owner GO** (merge `5694975`; QC project `35377356`, backtest `b9696f67…`): structurally complete first attempt, 54 months, all four policies, ledgered **R-029 UNANALYSED**, run-level look 29 → 30, no statistic observed. **APQ-5 OBSERVED 2026-08-19 under the owner GO (ledger A-003): NULL — 0 of 3 cells pass the 0.05/3 gate (P1 p=0.080, P2 p=0.121, P3 p=0.214), every excess mean negative; R-029 upgraded to VALID in the same commit; THE ALLOCATION-POLICY QC FAMILY IS CLOSED** (one run and one analyser pass, both spent; descriptive drawdown/Sharpe trade recorded without any claim; paper/live use of any weights is a separate owner decision on the Alpaca/REBAL stack). Interpretation limits recorded in the counter-review: the 2022+ window is regime-conditioned in hindsight (descriptive of that tape, not forward evidence), and the optional-test reporting decision is fixed at APQ-2 review per the plan's counter-review note. |
| LEV-SBR-20260819 | research | **Owner's original Strong-Buy/leverage direction split into two already frozen, separate contracts because QC has no point-in-time analyst ratings.** | **LEV-1 and SBR-1 are implemented and accepted after correction; neither validates the complete Strong-Buy portfolio.** LEV remains a generic TQQQ threshold-timing family. SBR remains capture-only on the frozen 102-symbol universe; its owner-present Interactive task install is unperformed. Under the currently adopted SBR contract, no rating snapshot may be joined to prices until a separate SBR-2 preregistration freezes after at least 12 captures. The draft SBP successor in the next row may replace that sequence only after explicit owner adoption; it has not done so yet. |
| STRONGBUY-PORTFOLIO-20260819 | research / portfolio testing | **Owner-requested complete strategy: every eligible stock passing a frozen Strong-Buy rule, inverse-volatility allocation, point-in-time ETF-holdings overlap, and a capped verified same-index leveraged sleeve.** | **DRAFT — TWO REVIEW ROUNDS ACCEPTED AFTER CORRECTION, BUT NOT ADOPTED, FROZEN, SCHEDULED, OR IMPLEMENTED.** First amendment/review: submitted `5c42bfd`, correction `5c3bf45`, record `docs/Archive/Review/REVIEW_2026-08-19_SBP_PLAN_AMENDMENTS.md` (five P2/two P3 closed). Counter-review verification: submitted `f75e793`, correction `aadb238`, record `docs/Archive/Review/REVIEW_2026-08-20_SBP_PLAN_COUNTERREVIEW_VERIFICATION.md` (one P2/two P3 closed). SBPA-006 is accepted as a proposal after clarification: official first-trading-date evidence plus the exchange calendar determines whether 64 completed closes are possible before ratings selection; provider row count may not decide eligibility, and broken data for an age-eligible selected security still refuses the month. The withdrawn 33.8% “ceiling,” 35–40% return, and 0.6%-monthly power claims remain non-evidence. Minimum basket is 10; exact price inputs are hash-bound; P3/P4 look-through uses the ordinary same-index exposure with correct 95% scaling; P4−P3 is descriptive; the inferential family is three one-sided cells at 0.05/3. The 10% overlap floor and all other section-2 values await owner adoption. Freeze SBP-0 before any admissible price join; pre-adoption snapshots are calibration-only. No code, QC, broker, scheduler, or deployment action is authorized. |
| ALPHA-QC-FULL-AUDIT-20260817 | research quality | **Owner-directed clean-slate review of all research/QC code and today's documentation.** | **CORRECTED, VALIDATED, MERGED BY PR #241 AT `d8a3260`, COUNTER-REVIEWED BY CLAUDE AT `ad3b3a8`, INDEPENDENTLY ACCEPTED BY CODEX AFTER A P3 RECORD CORRECTION, MERGED BY PR #242 AT `f937bfb`, AND FINALLY COUNTER-REVIEWED AGAIN BY AN INDEPENDENT CLAUDE SESSION ON 2026-08-17 (ALL TEN COMMITS ACCEPTED; FOUR P3 GAPS CCR3-A..D CLOSED); QC RERUN IS THE ONLY REMAINING RESEARCH STEP, PENDING THE OWNER'S STAGE-ORDER CALL.** Claude's counter-review verified the head with seven independent mutations: four detected, three surviving mutations became findings CR2-001..003 (all P3 test gaps over correct behaviour — the Stage 1 score-cutoff call site, the market-factor recorder's refusal, and Stage 1's own turnover copy were unpinned) and were closed with mutation-verified tests in the counter-review commit. Codex retained all three tests and corrected stale main/branch/worktree claims as CR2IR-001 on `codex/review-alpha-qc-counterreview-20260817`; see both counter-review records under `docs/Archive/Review/`. Corrections `855941a` and `1e2b631` repair the full LEAN/analyzer/runner surface and the older local turnover/NAV/peer/regression methodology. Every commit from `db0045a` through `a37e73b` has an explicit disposition. Invalid active artifacts were removed while the permanent ledger preserved identities and look counts; the docs tree was organized; a plain-language alpha glossary was added. Final pinned-environment validation after integrating the counter-review tests: 4,192 passed, 0 failed, 25 known warnings. No valid result or milestone exists, no QC access occurred, and the lifetime floor remains 428. The 2026-08-17 final counter-review re-verified the whole chain with sixteen mutations and closed four further P3 gaps (unpinned 24/428 multiplicity gates, the untested poll-deadline path, the incomplete LEAN legacy-name blocklist, and the ledger's undocumented CRLF hash convention). See `docs/Archive/Review/REVIEW_2026-08-17_ALPHA_QC_ROUND2.md` and `docs/Archive/Review/REVIEW_2026-08-17_ALPHA_QC_FINAL_COUNTERREVIEW.md`. |
| ALPHA-QC-FABLE-VERIFY-20260817 | research quality | **Owner request: verify Fable's merged final counter-review before starting QC.** | **COMPLETE AFTER CLAUDE COUNTER-REVIEW AND CODEX VERIFICATION; ALGORITHM CODE ACCEPTED AT PR #244.** Fable's exact three-commit range `5816f6f..6bd962f` was reviewed commit by commit after PR #243 merged it at `4151b3f`. Its tests are accepted, but its no-product-defect conclusion missed two P2 and two P3 Stage 0 defects. Correction `ac96d47` assigns each short holding its own entry plus exit turnover, infers the frozen 12/monthly and 42/short annualization cadence per family, requires an exact finite-positive MAX(20) window, and prevents missing industries from becoming a fake peer bucket. Claude's counter-review reproduced all four defects red on the pre-correction tree, ran seven mutations (one survivor became FCR-001: the drifted exit leg was unpinned on long/short books), found one surviving FQCV-004 instance in Stage 1's dead copied ingestion (FCR-002), and closed both with mutation-verified tests. PR #244 merged exact Claude head `9a7e9fc` at `b6f577e` with a byte-identical tree. Codex accepted all three commits and both closures; its only finding, FCRV-001 (P3), was a helper-only test that failed to pin the live industry ingestion call site, closed by test-only commit `39e5d99`. No QC access or look occurred; the cell/run ledgers remain 428/five. See `docs/Archive/Review/REVIEW_2026-08-17_ALPHA_QC_FABLE_COUNTERREVIEW.md`, `docs/Archive/Review/REVIEW_2026-08-17_ALPHA_QC_STAGE0_CORRECTION_COUNTERREVIEW.md`, and `docs/Archive/Review/REVIEW_2026-08-17_ALPHA_QC_STAGE0_COUNTERREVIEW_VERIFICATION.md`. |
| ALPHA-QC-STAGE0-LAUNCH-20260817 | research quality | **Owner selected Stage 0 and authorized the frozen QC runs.** | **BATTERY COMPLETE 2026-08-18, ALL NINE CELLS PENDING_REVIEW; NO STATISTIC OBSERVED.** The owner-directed serial rerun ("continue to test. 1 at a time") ran to completion: monthly R-009/R-011/R-012 (A/B/C), short R-014/R-015/R-016 (A/B/C), benchmark R-022/R-020/R-021 (A/B/C), every raw cloud log round-tripping the frozen parsers, ledgered with full identity in `docs/Archive/Research/alpha-result.md`. Run-level looks are now twenty-three. Two further defect classes surfaced and were fixed en route, both descendants of R-010's turnover-gating: R-013 (short A_large REFUSED — the packed v1 layout could not declare an absent spec-date, so one honest missing MAX_20 date withheld all 2,664 cells; fix `46221db` adds a per-date spec-presence mask and a turnover-unavailability sentinel, keeping v1 logs decodable) and R-017/R-019 (benchmark: the turnover-gated bind died silently at 2015-12 reporting 48 of ~156 months, then the all-names-priced settlement gate collapsed B_core coverage to 94/156 in a selectively calm pattern; fixes `5b5184a` and `39b3b89` make the bind unconditional and record underfill — priced AND entered counts per month — instead of dropping months). R-022's 149 months shared with the superseded R-018 replicate to 0.0 difference. **Independent review DONE 2026-08-18 (Cursor/Grok 4.6, substituting for token-exhausted Codex): full range `81db126..de1beac` dispositioned, all 27 commits accepted, eight findings (2 P2 blocking Stage 1 only, 6 P3), none invalidating the nine logs; Claude counter-review verified every finding — see `docs/Archive/Review/REVIEW_2026-08-18_QC_STAGE0_BATTERY_COMPLETION.md` and `docs/Archive/Review/REVIEW_2026-08-18_QC_STAGE0_BATTERY_COUNTERREVIEW.md`. Owner ACCEPTED the review pair 2026-08-18; the nine entries were upgraded to VALID (`2be903f`, committed before any statistic) and the frozen analysers ran ONCE (ledger entry A-001). RESULT: IC 0/44, long-short 0/44, short battery 0/60 — no beta-free cell cleared the Bonferroni gate (0.05/180); 6/88 long-only cells cleared the gross-vs-zero test, which carries market beta the equal-weight benchmarks (Sharpe 0.73–0.85) would also clear, so they are not evidence of selection edge; MULTI_ALPHA_COMPOSITE/A_large disclosed insufficient at 23<24 months. Stage 0 is evidence-complete. The S0R hardening round is IMPLEMENTED 2026-08-18 on `user/claude/qc-stage1-hardening-20260818` (S0R-001/002/003/004/005/008 closed, 8 reverse mutations verified, full suite now **4,246 passed / 0 failed / 25 warnings** after the owner enabled Windows long paths and the pinned streamlit==1.60.0 was restored — the 14 prior UI failures were machine-environment only, reproduced on the base commit and gone after the reinstall); The hardening round PASSED independent review 2026-08-18 (fresh Claude session, all 11 commits accepted, 0 P0-P2, eight mutations re-executed red, and the reviewer's own full-suite run independently reproduced 4,246/0/25 on the byte-identical tree; see `docs/Archive/Review/REVIEW_2026-08-18_S0R_HARDENING_REVIEW.md` — handoff 7ab's "unfilled placeholder" flag referred to a mid-review draft committed prematurely by another session and is corrected in handoff 7ac). Counter-review of that review completed 2026-08-18 and VERIFIED it (hashes, timing, probes, one fresh mutation — see `docs/Archive/Review/REVIEW_2026-08-18_S0R_HARDENING_COUNTERREVIEW.md`). A second Cursor/Grok round independently re-verified the full post-de1beac range 2026-08-18 (own mutations, all merges tree-checked, 0 P0-P2; its S0R2-001 stale-heading finding fixed same day) — the review chain is now cross-vendor. **STAGE 1 COMPLETE AND NULL 2026-08-18; THE CROSS-SECTIONAL ALPHA PROGRAM ON THIS UNIVERSE IS CLOSED (A-002).** Driver reviewed (zero findings), six serial runs R-023..R-028 all structurally complete first-attempt, frozen analyser run once: IC 0/6, long-short 0/6 (three negative Sharpes), and the 10/12 long-only stage-gate clears sit on their own same-dates benchmarks (0.84–1.01 vs 0.78–0.83) — beta, not selection. Preregistered consequence executed per the owner GO protocol: no further signal variants on this universe; lifetime cell floor now 452; effort redirects to portfolio construction, risk, and paper-observation infrastructure. Reopening would require a new owner decision, new universe/data, and fresh preregistration.** Earlier this round: fix `d305ea0` (turnover is a cost input that never gates a result row) closed R-010's zombie-name die-off; R-009 cloud-confirmed the R-007/R-008 fixes. Earlier history: the rerun round found two defects: R-007 (A_large) completed fully but its legitimate per-date spec subsets were unparseable under the frozen parser (STALE, rerun required with SPECMETA counts), and R-008 (B_core) died off after 2017-01 to an unrecoverable turnover-refusal spiral (INVALIDATED). Both fixes are implemented with a forced-skip-month simulation that pipes the algorithm's own log through the real parser; R-007 (A_large) completed fully but its legitimate per-date spec subsets were unparseable under the frozen parser (STALE, rerun required with SPECMETA counts), and R-008 (B_core) died off after 2017-01 to an unrecoverable turnover-refusal spiral (INVALIDATED). Both fixes are implemented with a forced-skip-month simulation that pipes the algorithm's own log through the real parser. Before that, both monthly A_large and B_core refused with the same three missing residual/composite specs, consumed two run-level looks, emitted zero cells, and left the 428-cell floor unchanged. Claude correctly stopped and found the weekend-label empty-bucket defect, but its first fix still applied a newly selected month's universe backward to the prior day's bar on ordinary first-trading-day ordering. Codex correction `2219643` snapshots both selected names and industries at each selection and preserves the prior snapshot for that return. It also prevents evidence overwrite, records returned cloud project identity, hashes exact log bytes, uses the canonical runtime-identity service, and corrects R-006's source commit/hash. See `docs/Archive/Review/REVIEW_2026-08-17_QC_STAGE0_LAUNCH_ROUND.md`. |
...
Implemented `docs/Archive/Plans/UI_FEATURE_CONTROLS_DESIGN.md` as one milestone:
...
`docs/Archive/Review/REVIEW_2026-08-03_PHASE2_HYGIENE.md`.
...
`docs/Archive/Review/REVIEW_2026-08-03_GR1E_ASSESSMENT.md`. Phase 4 later completed GR-3,
...
to `docs/Archive/Review/REVIEW_2026-08-03_CLAUDE_INTEGRITY_GR2.md`) confirmed every finding
...
`docs/Archive/Operations/PHASE5_DEPLOYMENT_SESSION.md` for results and the ordered owner
...
`docs/Archive/Review/REVIEW_2026-08-06_FULL_PROJECT_SWEEP.md`,
`docs/Archive/Review/REVIEW_2026-08-06_FULL_PROJECT_SWEEP_INDEPENDENT.md`, and
...
The archived plan (`docs/Plan/GENERAL_READINESS_IMPLEMENTATION_PLAN.md`
...
`docs/Plan/THREE_SLEEVE_ENGINE_PLAN.md`, which now governs this
