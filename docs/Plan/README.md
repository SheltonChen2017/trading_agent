...
track**, not one overall. There are currently two tracks: the research track
uses the coordinated contracts under `docs/Strategy Description/`, while the
architecture track keeps `PROJECT_SEPARATION_IMPLEMENTATION_PLAN.md` at the
root. Any additional root plan means either a new track the Action Plan must
name or a plan that should have been archived. The allowlist in
`tests/test_active_document_consistency.py` enforces root membership; this
paragraph is the rule it enforces, so the rule cannot be changed by editing
the test alone.
