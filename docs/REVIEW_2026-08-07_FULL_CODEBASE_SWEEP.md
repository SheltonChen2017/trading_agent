...
**No P0. No P1. Four confirmed P2 defects, all reproduced. Thirteen P3.**

**Status: FCS-001 and FCS-016 are FIXED on this branch** (see their ledger rows for the
correction and its red/green verification). FCS-002, FCS-003 and every P3 remain open.
...
- Full suite on the **exact final tree** after the FCS-001 / FCS-016 fixes:
  **3041 passed, 0 failed, 0 skipped, 25 warnings** (352s). The +26 is exactly
  the tests added by those two fixes (18 in `test_tax_lots.py`, 8 in
  `test_strategy_proposals.py`); no pre-existing test changed its result.
...
- Reverse mutations, each applied in the fixed code's own location and then
  ...
  back) → **7 fail**. Both restored green.
- FCS-008 mutation: applied, detected by 2 tests, reverted; `git status`
  verified clean afterwards.
- FCS-002, FCS-003, FCS-005, FCS-012, FCS-017 reproduced in the interpreter
  against the real modules; none of them is fixed.
