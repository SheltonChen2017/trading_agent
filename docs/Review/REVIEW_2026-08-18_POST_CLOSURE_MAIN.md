...
| POST-002 | P3 | Open | `9ba7d06` / `98243a1` | `docs/reference/SHADOW_OBSERVATION_DESIGN.md` §2; handoff §7ak | Design claims import-boundary tests “extend to prove both directions” and that `overlay_shadow.py` holds “pure computation.” | `tests/test_ml_import_boundary.py` still only walks `ml` imports. `overlay_shadow.py` is contracts only. | Overclaim will be copied as if SHW-1 already proved the overlay graph and overlay math. | Narrow the sentences, or add overlay↔execution AST tests and keep computation for SHW-2. | Test fails if `execution/` imports `overlay_shadow`. |
