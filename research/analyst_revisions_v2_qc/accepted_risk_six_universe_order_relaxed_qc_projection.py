...
def _render_source(path, source, percent, policy):
    if path == _GATE_PATH:
        source = _selection.render_gate_source(source, policy)
    elif path == _TARGET_PATH:
        source = _selection.render_targets_source(source)
    ...
    if path not in _VERSIONED_PATHS:
        return source
    replacements = {
        "matched_revision_tilt100_recent": f"matched_revision_tilt{percent}_relaxed_recent",
        "cap90_matched_revision_tilt100_recent_v1": f"cap90_matched_revision_tilt{percent}_relaxed_recent_v1",
        "cap90_exploratory_v3": "cap90_relaxed_recent_v1",
        "cap90_admission_settlement_v1": "cap90_admission_relaxed_recent_v1",
    }
            ...
            if type(node.value) is str:
                ...
                if value in replacements:
                    ...
                    if ("relaxed-selection" not in value and "order-relaxed-sleeve" not in value
                        ...
                        node.value = (value + "relaxed-") if value.endswith("-") else value + "-relaxed-v1"
            return node

        def visit_ClassDef(self, node):
            if path == "main.py" and node.name == "ARV2SixUniverseOrderTilt100RecentAlgorithm":
                node.name = f"ARV2SixUniverseOrderTilt{percent}RelaxedRecentAlgorithm"
...
def build_relaxed_order_projection(prior_package, latest_package, percent, policy):
    ...
    sources = {item.project_path: _render_source(item.project_path,
        item.source_bytes.decode("ascii"), percent, policy) for item in predecessor.source_files}
    ...
    with _cloud_loader(sources) as (load, _modules):
        ...
        if runtime.TILT_SUMMARY_SCHEMA != SUMMARY_SCHEMAS[percent] or runtime.TILT_META_SCHEMA != META_SCHEMA:
    ...
    value = dataclasses.replace(predecessor,
        schema=f"arv2-six-universe-relaxed-tilt{percent}-qc-projection-v1",
        role=profile["role"], variant=f"cap90_matched_revision_tilt{percent}_relaxed_recent_v1",
    ...
    return dataclasses.replace(value, projection_sha256=digest,
        projection_id=f"arv2-six-universe-relaxed-tilt{percent}-qc-projection-" + digest[:24]), json.loads(_base._canonical(profile).decode("ascii"))
