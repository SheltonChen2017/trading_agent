...
CONTRACT_SCHEMA = "arv2-qc-pit-market-cap-membership-coverage-contract-v2"
PLAN_SCHEMA = "arv2-qc-pit-market-cap-membership-coverage-plan-v2"
PROJECTION_SCHEMA = "arv2-qc-pit-market-cap-membership-coverage-projection-v1"
SOURCE_FILE_SCHEMA = "arv2-qc-pit-market-cap-membership-project-source-v1"
RECEIPT_SCHEMA = "arv2-qc-pit-market-cap-membership-coverage-receipt-v2"
TERMINAL_POINTER_SCHEMA = (
    "arv2-qc-pit-market-cap-membership-coverage-terminal-v2"
)
FAILURE_SCHEMA = "arv2-qc-pit-market-cap-membership-coverage-failure-v2"
ATTESTATION_SCHEMA = (
...
ENTRY_PATH = "main.py"
RUNTIME_PATH = "pit_market_cap_membership_probe_runtime.py"
...
SUMMARY_NAME = "ARV2_PIT_MARKET_CAP_MEMBERSHIP_COVERAGE"
INPUT_PREFIX = "arv2/pit-market-cap-membership-coverage/input/"
OUTPUT_PREFIX = "arv2/pit-market-cap-membership-coverage/output/"
ETFS = ("SPY", "QQQ", "SOXX")
...
_MARKER = "__ARV2_PIT_MARKET_CAP_MEMBERSHIP_CONTRACT_SHA256__"
...
CONTRACT_ID = "arv2-pit-market-cap-membership-contract-" + CONTRACT_SHA256[:24]
...
def build_pit_market_cap_membership_probe_plan_bytes(
    *, decision_sessions: Sequence[str], calculation_session: str
    ...
    base["plan_id"] = "arv2-pit-market-cap-membership-plan-" + seed[:24]
...
def _audit_source_capabilities(
    ...
    allowed_imports = {
        ...
        "decimal",
        "zoneinfo",
        "pit_market_cap_membership_probe_runtime",
    }
    forbidden_names = {
        "TradeBar",
        "QuoteBar",
...
import json
from pit_market_cap_membership_probe_runtime import execute_pit_market_cap_membership_probe
...
def _source_set_hash(files: tuple[PitCoverageProbeProjectSource, ...]) -> str:
    return hashlib.sha256(
        canonical_json_bytes(
            {
                "schema": "arv2-qc-pit-market-cap-membership-source-set-v1",
...
def build_pit_market_cap_membership_probe_qc_projection(
    *, plan_bytes: bytes, runtime_source_bytes: bytes
    ...
    fields = {
        **record,
        "projection_id": "arv2-pit-market-cap-membership-projection-"
        + digest[:24],
...
def require_pit_market_cap_membership_probe_qc_projection(
    ...
    if (
        ...
        != "arv2-pit-market-cap-membership-projection-" + digest[:24]
...
def _validate_attestation_availability(
    ...
    for ticker in ETFS:
        ...
        earliest = _local_timestamp(
            item["earliest_collection_end_time_local"],
            f"{ticker} earliest collection EndTime",
        )
        latest = _local_timestamp(
            item["latest_collection_end_time_local"],
            f"{ticker} latest collection EndTime",
        )
        if (
...
def _load_completed_attestation(
    *,
    ...
    if (
        ...
        or re.fullmatch(
            r"arv2-pit-market-cap-membership-coverage-[0-9a-f]{24}", receipt_id
        )
        is None
    ):
...
def load_reviewed_pit_market_cap_membership_probe_receipt(
    *,
    ...
    if (
        ...
        != "arv2-pit-market-cap-membership-coverage-" + semantic_sha256[:24]
    ...
    return ReviewedPitMarketCapMembershipCoverageReceipt(
        schema="arv2-reviewed-pit-market-cap-membership-coverage-receipt-v2",
