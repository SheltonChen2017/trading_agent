...
CONTRACT_SHA256 = "__ARV2_PIT_MARKET_CAP_MEMBERSHIP_CONTRACT_SHA256__"
...
TERMINAL_POINTER_SCHEMA = "arv2-qc-pit-market-cap-membership-coverage-terminal-v2"
...
def _fundamental_snapshot(rows):
    try:
        ...
        raise ValueError("fundamental collection is unreadable") from exc
    if not members or len(members) > MAX_COLLECTION_ROWS:
...
def _constituent_snapshot(rows):
    try:
        ...
        raise ValueError("ETF constituent collection is unreadable") from exc
    if not members or len(members) > MAX_COLLECTION_ROWS:
...
def _identified_receipt(record):
    ...
    record["receipt_id"] = "arv2-pit-market-cap-membership-coverage-" + digest[:24]
...
def _run(algorithm, constants):
    ...
    for chunk in plan["history_chunks"]:
        ...
        for session in chunk["decision_sessions"]:
            ...
            for ticker in ETFS:
                ...
                etf_rows[ticker] = {
                    "collection_end_time_local": collection_time.isoformat(
    ...
    availability_extrema = {
        ...
        "etfs": {
            ticker: {
                "earliest_collection_end_time_local": min(
                    row["etfs"][ticker]["collection_end_time_local"]
                    for row in session_censuses
                ),
                "latest_collection_end_time_local": max(
                    row["etfs"][ticker]["collection_end_time_local"]
                    for row in session_censuses
                ),
            }
