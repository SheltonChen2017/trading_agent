"""One-use private QC launch and aggregate read for the R186 80% tilt.
...
from . import accepted_risk_six_universe_order_tilt80_qc_projection as tilt80_projection
...
class SixUniverseTilt80SubmissionError(ValueError):
...
_PROJECT_NAME = "109 ARV2 SIX CAP90 TILT80 R186 2021 2025"
_ROLE = "matched_revision_tilt80"
_PROJECTION_SHA256 = "16240a1ead1dd8466961556ffcfcfc58a6c974d763e87b72c27cfdce72699604"
_PROFILE_SHA256 = "2fe851dd13421ea0d7d750d17ba509ee6f8407e25eb6a142725cf7c17f387036"
_SOURCE_FILES_SHA256 = "78a650aa270e830e4f8194df5c32e32f3f9d4d0e9e6bd280d6180d4d4c37e02c"
_BACKTEST_NAME = "ARV2 R186A1 six cap90 bridge tilt80 2021 2025 " + _PROJECTION_SHA256[:8]
...
_LAUNCH_PERMIT_SCHEMA = "arv2-six-universe-tilt80-owner-launch-permit-v1"
_WAIVER_SCHEMA = "arv2-six-universe-tilt80-exact-owner-waiver-v1"
_WAIVER_ID = "ARV2-OWNER-2026-09-25-R186A1-TILT80-EXPLORATORY-SIGNATURE-WAIVER"
...
def _fail(message: str):
    raise SixUniverseTilt80SubmissionError(message)
...
def _post(api: QuantConnectClient, endpoint: str, payload: dict) -> dict:
    try:
        ...
        raise SixUniverseTilt80SubmissionError(str(exc)) from None
...
def _control_path(plan: Tilt80QcPlan, name: str) -> Path:
...
def _require_plan(plan: Tilt80QcPlan) -> None:
    """Keep status and result reads on the same exact R186 launch identity."""
    if type(plan) is not Tilt80QcPlan or (
        plan.candidate_id != "R186" or type(plan.attempt) is not int
        ...
        _fail("tilt80 launch plan changed from the exact R186 A1 identity")


def preview(plan: Tilt80QcPlan, projection: object) -> dict:
    """Authenticate the pinned, 16-file R186 A1 source before any QC call."""
    _require_plan(plan)
    profile = tilt80_projection.require_tilt80_profile()
    if (
        type(projection) is not base_projection.AcceptedRiskSixUniverseOrderQcProjection
        or projection.schema != tilt80_projection.PROJECTION_SCHEMA
        or projection.role != plan.role
        or projection.variant != tilt80_projection.TILT80_VARIANT
    ...
    if (
        ...
        or projection.projection_id != (
            "arv2-six-universe-order-tilt80-qc-projection-" + digest[:24]
        )
    ...
    return {
        "candidate_id": "R186", "attempt": 1, "role": plan.role,
...
def _require_valid_predecessors(plan: Tilt80QcPlan) -> str:
    ...
    if target_path_sha256 != _PREDECESSOR_TARGET_PATH_SHA256:
        _fail("tilt80 R182 target path changed from its preregistered pin")
    return target_path_sha256


def _render_owner_launch_payload(
    plan: Tilt80QcPlan, identity: dict, *, matched_baseline_target_path_sha256: str,
) -> bytes:
    if (
...
def render_owner_launch_permit(plan: Tilt80QcPlan, projection: object) -> bytes:
...
def load_owner_launch_permit(
    plan: Tilt80QcPlan, projection: object, *,
    allowed_signers_path: Path, signature_path: Path,
) -> OwnerSignatureAuthority:
    """Verify owner-controlled files against this R186 launch's exact bytes."""
    ...
    try:
        ...
        raise SixUniverseTilt80SubmissionError(
            "tilt80 owner launch signature is unavailable"
        ) from exc


def _require_owner_launch_permit(
    plan: Tilt80QcPlan, identity: dict, matched_path_sha: str,
    ...
    if (
        ...
        _fail("tilt80 owner launch signature identity changed")
    return {
...
def _launch_authority(
    ...
    """Require exactly one R186 A1 authority mode, before network or claim."""
    if (owner_signature is None) == (owner_waiver_id is None):
        _fail("tilt80 launch needs exactly one owner signature or exact waiver")
    ...
    if type(owner_waiver_id) is not str or owner_waiver_id != _WAIVER_ID:
        _fail("tilt80 exploratory owner signature waiver does not cover launch")
...
def launch_a1(
    plan: Tilt80QcPlan, projection: object, api: QuantConnectClient, *,
    ...
    if state["state"] == "BuildError":
        _write(_control_path(plan, "terminal"), {
            "candidate_id": "R186", "status": "BuildError",
...
def _check_uploaded_source(plan: Tilt80QcPlan, project_id: int,
...
def _match_launch(plan: Tilt80QcPlan, launch: dict) -> None:
    _require_plan(plan)
    if type(launch) is not dict or (
        launch.get("candidate_id") != "R186"
...
def poll_status(plan: Tilt80QcPlan, launch: dict, api: QuantConnectClient) -> str:
    ...
    if status in {"Completed.", "Runtime Error"}:
        _write(terminal_path, {
            "candidate_id": "R186", "status": status,
...
def read_aggregates_once(plan: Tilt80QcPlan, launch: dict,
    ...
    if _read(_control_path(plan, "terminal")) != {
        "candidate_id": "R186", "status": "Completed.",
        "project_id": launch["project_id"],
        "backtest_id": launch["backtest_id"],
    }:
    ...
    if (
        ...
        or claim.get("profile_id") != tilt80_projection.require_tilt80_profile()["profile_id"]
        ...
        _fail("tilt80 source claim or predecessor changed")
    signature_keys = (
    ...
    if claim.get("owner_launch_authority_mode") == "exact_exploratory_signature_waiver":
        ...
        if (
            ...
            _fail("tilt80 exact owner waiver receipt changed")
    elif (
        ...
        _fail("tilt80 owner signature receipt changed")
    _check_uploaded_source(plan, launch["project_id"], claim, api)
    _write(read_path, {
        "candidate_id": "R186", "project_id": launch["project_id"],
        "backtest_id": launch["backtest_id"],
    })
    ...
    if (
        ...
        or meta.get("aggregate_schema") != tilt80_projection.TILT80_SUMMARY_SCHEMA
        ...
        or aggregate.get("schema") != tilt80_projection.TILT80_SUMMARY_SCHEMA
        ...
        or aggregate.get("tilt_rank_rule_id") != tilt80_projection.TILT80_RANK_RULE_ID
    ...
    if valid:
        _write(_control_path(plan, "result-valid"), {
            "candidate_id": "R186", "attempt": 1, "run_valid": True,
...
__all__ = (
    "SixUniverseTilt80SubmissionError", "Tilt80QcPlan", "launch_a1",
