...
strategy, applied to SOXX/SOXL (semiconductor index) instead of QQQ/TQQQ.

QQQ/TQQQ passed its own walk-forward test (2/3 folds) BEFORE a same-close
look-ahead bug was found and fixed — after the fix, it dropped to only
1/3 folds beating baseline on both axes (see memory: project_leverage_
rotation_strategy.md). SOXX/SOXL's single discovery/confirmation
confirmation survived that same fix almost unchanged, but has NOT yet
been through walk-forward — this is that missing check, run under the
already-corrected (next-day-open) execution timing from the start.

Same procedure as run_regime_rotation_walkforward.py: calibrate vol
threshold from discovery only, grid-search state weights on discovery
only, apply unchanged to confirmation, across THREE non-overlapping
folds with an expanding discovery window.
"""
import sys
