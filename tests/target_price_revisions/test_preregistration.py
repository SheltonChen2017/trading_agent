...
def _bare_tmp_path_refusal(tmp_path: Path) -> str:
    ...
    worktree, where the spec is an uncommitted file; any other repository above
    it cannot be the registry's.  Mirrors the loader's discovery order so the
...
def test_self_declared_review_and_registry_substitution_refuse(
    ...
    # review is an uncommitted file and is refused as such.
    uncommitted = _write(spec_path.parent, raw, "self-reviewed.json")
    with pytest.raises(PreregistrationError, match="committed and clean"):
        load_reviewed_algorithm_spec(uncommitted)
    uncommitted.unlink()
