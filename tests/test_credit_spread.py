"""
Sanity tests for signals/credit_spread.py. Run with:
...
def test_flags_entire_universe_dip_on_spread_widening_spike():
    days = 60
    spread_df = _flat_series_with_shock(days, shock_index=days - 1, shock_return=0.35)
    as_of = spread_df.index[-1]
    data = {
        "AAA": _flat_stock_data(spread_df.index, close=50.0),
        "BBB": _flat_stock_data(spread_df.index, close=200.0),
    }

    result = scan_credit_spread(data, spread_data=spread_df, as_of=as_of)
...
def test_flags_entire_universe_up_on_spread_narrowing():
    days = 60
    spread_df = _flat_series_with_shock(days, shock_index=days - 1, shock_return=-0.30)
    as_of = spread_df.index[-1]
    data = {"AAA": _flat_stock_data(spread_df.index)}
...
def test_no_signal_when_as_of_is_none():
    ...
    spread_df = _flat_series_with_shock(days, shock_index=days - 1, shock_return=0.35)
    data = {"AAA": _flat_stock_data(spread_df.index)}
    result = scan_credit_spread(data, spread_data=spread_df, as_of=None)
    assert result.empty


def test_no_signal_when_as_of_missing_from_spread_index():
    days = 60
    spread_df = _flat_series_with_shock(days, shock_index=days - 1, shock_return=0.35)
    data = {"AAA": _flat_stock_data(spread_df.index)}
    missing_date = pd.Timestamp("1999-01-01")
    result = scan_credit_spread(data, spread_data=spread_df, as_of=missing_date)
    assert result.empty
...
def test_skips_ticker_missing_on_as_of_date():
    ...
    spread_df = _flat_series_with_shock(days, shock_index=days - 1, shock_return=0.35)
    as_of = spread_df.index[-1]
    full_history = _flat_stock_data(spread_df.index)
    short_history = full_history.iloc[:-1]

    result = scan_credit_spread({"FULL": full_history, "SHORT": short_history}, spread_data=spread_df, as_of=as_of)
    assert "FULL" in result["ticker"].values
    assert "SHORT" not in result["ticker"].values


if __name__ == "__main__":
    test_flags_entire_universe_dip_on_spread_widening_spike()
    test_flags_entire_universe_up_on_spread_narrowing()
    test_no_signal_on_quiet_day()
    test_no_signal_when_as_of_is_none()
    test_no_signal_when_as_of_missing_from_spread_index()
    test_skips_ticker_missing_on_as_of_date()
    print("All credit_spread signal tests passed.")
